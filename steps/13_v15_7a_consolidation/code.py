...
# ===========================================================================
# v15.7a PAS 7A — Section C1: LongitudinalEpisodeRegime
# ===========================================================================
#
# Scope: regime-level test infrastructure for cross-episode consolidation.
#
# All existing metrics (F1-F5, S5, S6) test decisions within ONE episode and
# reset the bank between trials. Consolidation cannot be measured under that
# regime. This section adds a sequence-level regime where:
#   - Each sequence shares a single `primary_entity_id` across episodes
#   - Bank and ProvisionalMemory persist across episodes inside a sequence
#     (they ARE reset between sequences)
#   - The consolidator will run at end_episode inside the sequence (wired
#     in section C4; not yet active at this point)
#
# This section defines DATA ONLY: sequences, generators, expected-state
# predictions. Execution and scoring live in later sections.
#
# Parameters approved for Pas 7a (see step README for rationale):
#   - N_promote = 2         (distinct episode_ids)
#   - M_retrograde = 2      (distinct episode_ids)
#   - K_promote_age = 2     (episoade)
#   - K_prune_stale = 3     (episoade fără activitate)
# ===========================================================================


@dataclass
class LongitudinalSequence:
    """A consecutive run of episodes sharing primary_entity_id.

    Expected state fields are PREDICTIONS based on architectural design.
    They become falsifiable once the consolidator is wired (D.8).
    """
    sequence_id:                   str
    family_tag:                    str
    primary_entity_id:             str
    episodes:                      List["HoldoutEpisode"]
    # Predicted state at end of final episode
    expected_final_committed:      Dict[Tuple[str, str], int]
    expected_provisional_present:  List[Tuple[str, str, int]]
    expected_provisional_absent:   List[Tuple[str, str, int]]
    # Predicted consolidator ops summed across all end_episode calls in sequence
    expected_ops:                  Dict[str, int]
    description:                   str


def _v15_7a_build_episode(fact_texts:             List[str],
                             query_entity_id:     str,
                             query_attr_type:     str,
                             query_target_value_idx: Optional[int],
                             class_map:           Dict[str, int],
                             family_tag:          str,
                             ep_idx:              int,
                             target_is_unknown:   bool = False
                             ) -> "HoldoutEpisode":
    """Build one HoldoutEpisode from a list of fact strings.

    The query always probes (query_entity_id, query_attr_type). Facts can
    refer to different entities/attrs (the arbiter re-parses every fact).
    """
    attr_idx = HOLDOUT_ATTR_TYPES.index(query_attr_type)
    if target_is_unknown or query_target_value_idx is None:
        target_tok = 0
    else:
        value_str = HOLDOUT_ATTR_VALUES[query_attr_type][query_target_value_idx]
        target_tok = _tok_first(ENC, value_str)

    query_templates = {
        "color":    f"What color is the {query_entity_id}? The {query_entity_id} is",
        "size":     f"What size is the {query_entity_id}? The {query_entity_id} is",
        "location": f"Where is the {query_entity_id}? The {query_entity_id} is in the",
        "state":    f"What state is the {query_entity_id} in? The {query_entity_id} is",
    }

    # Placeholder classifier fields; actual writing goes through arbiter
    # which re-parses every fact.
    n = len(fact_texts)
    return HoldoutEpisode(
        episode_type=f"longitudinal_{family_tag}_ep{ep_idx}",
        family_tag=family_tag,
        facts=list(fact_texts),
        fact_entity_tokens=[_tok_first(ENC, query_entity_id)] * n,
        fact_attr_labels=[attr_idx] * n,
        fact_answer_tokens=[target_tok] * n,
        fact_class_labels=[class_map.get(query_entity_id, 0)] * n,
        fact_is_anchor=[False] * n,
        query=query_templates[query_attr_type],
        query_attr_label=attr_idx,
        query_entity_token=_tok_first(ENC, query_entity_id),
        target_answer_token=target_tok,
        target_is_unknown=target_is_unknown,
        target_fact_idx=0,
        target_slot_name=query_attr_type,
    )


# ---------- L1: promote_cycle (retrograde at ep3, promote at ep4) ----------
def gen_L1_promote_cycle(rng, enc, class_map) -> LongitudinalSequence:
    """Full promote cycle. The intra-pas block forces promote at ep4.

    ep1: commit stable_val (bank: stable). Provisional: empty.
    ep2: challenger (cross-episode). Provisional: {chall:{ep2}}.
    ep3: challenger again. Provisional reconciles to {chall:{ep2,ep3}}.
         end_ep3 consolidator: M=2 met → RETROGRADE stable.
         Promote BLOCKED (intra-pas exclusion after retrograde same slot).
    ep4: distractor (unrelated entity).
         end_ep4 consolidator: blue has N=2, age=2, 0 challengers → PROMOTE.
    """
    entity = rng.choice(HOLDOUT_ENTITIES_SINGLE)
    attr = "color"
    stable_val = rng.choice(HOLDOUT_COLORS)
    chall_val = rng.choice([v for v in HOLDOUT_COLORS if v != stable_val])
    stable_idx = HOLDOUT_COLORS.index(stable_val)
    chall_idx = HOLDOUT_COLORS.index(chall_val)

    distractor_entity = rng.choice(
        [e for e in HOLDOUT_ENTITIES_SINGLE if e != entity]
    )
    distractor_size = rng.choice(HOLDOUT_SIZES)

    episodes = [
        _v15_7a_build_episode(
            [f"The {entity} is {stable_val}."],
            entity, attr, stable_idx, class_map,
            "L1_promote_cycle", 1,
        ),
        _v15_7a_build_episode(
            [f"The {entity} is {chall_val}."],
            entity, attr, stable_idx, class_map,   # query expects stable (bank still red)
            "L1_promote_cycle", 2,
        ),
        _v15_7a_build_episode(
            [f"The {entity} appeared {chall_val} nearby."],
            entity, attr, stable_idx, class_map,   # query at ep3 before consolidator: still red
            "L1_promote_cycle", 3,
        ),
        _v15_7a_build_episode(
            [f"The {distractor_entity} is {distractor_size}."],
            entity, attr, None, class_map,
            "L1_promote_cycle", 4,
            target_is_unknown=True,  # bank on (entity, color) is empty right after ep3 retrograde
        ),
    ]
    return LongitudinalSequence(
        sequence_id=f"L1_{entity}_{stable_val}_to_{chall_val}",
        family_tag="L1_promote_cycle",
        primary_entity_id=entity,
        episodes=episodes,
        expected_final_committed={(entity, attr): chall_idx},
        expected_provisional_present=[],
        expected_provisional_absent=[
            (entity, attr, chall_idx),    # promoted out of provisional at ep4
            (entity, attr, stable_idx),   # retrograded at ep3; NOT re-added to provisional in v1
        ],
        expected_ops={"RECONCILE": 1, "RETROGRADE": 1, "PROMOTE": 1},
        description=(
            "retrograde at end_ep3 (M=2 distinct-episode challengers); "
            "promote at end_ep4 (N=2, age=2, no challengers; delayed 1 ep "
            "by intra-pas exclusion after retrograde)"
        ),
    )


# ---------- L2: retrograde only (no promote stage) ----------
def gen_L2_retrograde_only(rng, enc, class_map) -> LongitudinalSequence:
    """Retrograde fires at end_ep3. No distractor episode, no promote observed."""
    entity = rng.choice(HOLDOUT_ENTITIES_SINGLE)
    attr = "color"
    stable_val = rng.choice(HOLDOUT_COLORS)
    chall_val = rng.choice([v for v in HOLDOUT_COLORS if v != stable_val])
    ...
    episodes = [
        _v15_7a_build_episode(
            [f"The {entity} is {stable_val}."],
            entity, attr, stable_idx, class_map,
            "L2_retrograde_only", 1,
        ),
        _v15_7a_build_episode(
            [f"The {entity} is {chall_val}."],
            entity, attr, stable_idx, class_map,
            "L2_retrograde_only", 2,
        ),
        _v15_7a_build_episode(
            # Post-copular form: {chall_val} appears AFTER the copula "stood"
            # so RoMR classifies it as ATTRIBUTE_VALUE (not ENTITY_MODIFIER).
            # Original "A {chall_val} {entity} stood nearby." placed the value
            # in NP interior (premodifier of {entity}) which RoMR correctly
            # filters out under Pas 6, suppressing the second challenger
            # episode. Reordering preserves the same lexical content (stood,
            # nearby) but yields a parseable post-copular fact.
            [f"The {entity} stood {chall_val} nearby."],
            entity, attr, stable_idx, class_map,
            "L2_retrograde_only", 3,
        ),
    ]
    return LongitudinalSequence(
        sequence_id=f"L2_{entity}_{stable_val}_contested_by_{chall_val}",
        family_tag="L2_retrograde_only",
        primary_entity_id=entity,
        episodes=episodes,
        expected_final_committed={},    # (entity, attr) cleared after retrograde
        expected_provisional_present=[(entity, attr, chall_idx)],
        expected_provisional_absent=[(entity, attr, stable_idx)],
        expected_ops={"RECONCILE": 1, "RETROGRADE": 1},
        description=(
            "retrograde at end_ep3 (M=2 distinct challengers). "
            "Sequence ends before promote window (ep4 not present)."
        ),
    )


# ---------- L3: completion (different attrs over time, zero consolidator ops) ----------
def gen_L3_completion(rng, enc, class_map) -> LongitudinalSequence:
    """Same entity gets 3 different attributes in 3 episodes.
    No conflict, no provisional. Consolidator no-op on every slot.
    Gate 7 verifies: false_retrograde = 0 on completions.
    """
    entity = rng.choice(HOLDOUT_ENTITIES_SINGLE)
    color_val = rng.choice(HOLDOUT_COLORS)
    size_val = rng.choice(HOLDOUT_SIZES)
    loc_val = rng.choice(HOLDOUT_LOCATIONS)
    color_idx = HOLDOUT_COLORS.index(color_val)
    size_idx = HOLDOUT_SIZES.index(size_val)
    loc_idx = HOLDOUT_LOCATIONS.index(loc_val)

    episodes = [
        _v15_7a_build_episode(
            [f"The {entity} is {color_val}."],
            entity, "color", color_idx, class_map,
            "L3_completion", 1,
        ),
        _v15_7a_build_episode(
            [f"The {entity} is {size_val}."],
            entity, "size", size_idx, class_map,
            "L3_completion", 2,
        ),
        _v15_7a_build_episode(
            [f"The {entity} is in the {loc_val}."],
            entity, "location", loc_idx, class_map,
            "L3_completion", 3,
        ),
    ]
    return LongitudinalSequence(
        sequence_id=f"L3_{entity}_completion",
        family_tag="L3_completion",
        primary_entity_id=entity,
        episodes=episodes,
        expected_final_committed={
            (entity, "color"):    color_idx,
            (entity, "size"):     size_idx,
            (entity, "location"): loc_idx,
        },
        expected_provisional_present=[],
        expected_provisional_absent=[],
        expected_ops={},   # zero consolidator ops anywhere
        description=(
            "3 different attrs on same entity across 3 episodes; "
            "no conflict, no provisional, consolidator is no-op on every slot. "
            "Gate 7: no false retrograde."
        ),
    )


# ---------- L4: no_inflation (intra-ep repetition must not inflate confirmations) ----------
def gen_L4_no_inflation(rng, enc, class_map) -> LongitudinalSequence:
    """ep1 has 3 facts: two identical value (red), one different (blue).
    Same-episode conflict → all to provisional.
    Reconcile at end_ep1 collapses duplicates of red to 1 entry.
    Result: {red:{ep1}, blue:{ep1}} — both 1 distinct episode.

    ep2, ep3: distractors. No activity on (entity, color).

    Expected: NEITHER red NOR blue is promoted anywhere. Prune not yet
    triggered (K=3 requires end_ep4 at earliest for an ep1 entry; ep3
    gives diff=2, not stale).

    Gate 8: promote_count = 0 when confirmations come from same episode.
    """
    entity = rng.choice(HOLDOUT_ENTITIES_SINGLE)
    attr = "color"
    val1 = rng.choice(HOLDOUT_COLORS)
    val2 = rng.choice([v for v in HOLDOUT_COLORS if v != val1])
    val1_idx = HOLDOUT_COLORS.index(val1)
    val2_idx = HOLDOUT_COLORS.index(val2)

    distractor_entity = rng.choice(
        [e for e in HOLDOUT_ENTITIES_SINGLE if e != entity]
    )
    distractor_state = rng.choice(HOLDOUT_STATES)

    episodes = [
        _v15_7a_build_episode(
            [
                f"The {entity} is {val1}.",
                f"A {val1} {entity} appeared.",
                f"The {entity} is {val2}.",
            ],
            entity, attr, None, class_map,
            "L4_no_inflation", 1,
            target_is_unknown=True,   # empty-slot conflict → committed stays empty
        ),
        _v15_7a_build_episode(
            [f"The {distractor_entity} is {distractor_state}."],
            entity, attr, None, class_map,
            "L4_no_inflation", 2,
            target_is_unknown=True,
        ),
        _v15_7a_build_episode(
            [f"A {distractor_state} {distractor_entity} rested there."],
            entity, attr, None, class_map,
            "L4_no_inflation", 3,
            target_is_unknown=True,
        ),
    ...
    return LongitudinalSequence(
        sequence_id=f"L4_{entity}_{val1}_{val2}_no_inflation",
        family_tag="L4_no_inflation",
        primary_entity_id=entity,
        episodes=episodes,
        expected_final_committed={},   # (entity, color) never committed
        expected_provisional_present=[
            (entity, attr, val1_idx),   # red: 1 distinct episode, N=1, no promote
            (entity, attr, val2_idx),   # blue: same
        ],
        expected_provisional_absent=[],
        expected_ops={"RECONCILE": 1},
        description=(
            "intra-ep1 duplicates of red collapse to 1 entry via reconcile. "
            "Across ep2-ep3, no activity on (entity, color). "
            "NEITHER value promoted (both have 1 distinct episode, N=2 required). "
            "Gate 8: no inflation from same-episode repetition."
        ),
    )


# ---------- L5: stale_prune (provisional without activity for K=3 → prune) ----------
def gen_L5_stale_prune(rng, enc, class_map) -> LongitudinalSequence:
    """ep1 creates provisional via conflict. ep2-ep4 have no activity on
    (entity, color). At end_ep4, inactivity = 3 episodes (ep2, ep3, ep4) →
    K_prune_stale=3 reached → PRUNE both provisional entries.
    ep5 verifies provisional stays empty after prune.
    """
    entity = rng.choice(HOLDOUT_ENTITIES_SINGLE)
    attr = "color"
    val1 = rng.choice(HOLDOUT_COLORS)
    ...
    other_entities = [e for e in HOLDOUT_ENTITIES_SINGLE if e != entity]
    distractors = rng.sample(other_entities, 4) if len(other_entities) >= 4 else other_entities * 2

    episodes = [
        _v15_7a_build_episode(
            [f"The {entity} is {val1}.", f"The {entity} is {val2}."],
            entity, attr, None, class_map,
            "L5_stale_prune", 1,
            target_is_unknown=True,
        ),
        _v15_7a_build_episode(
            [f"The {distractors[0]} is {rng.choice(HOLDOUT_SIZES)}."],
            entity, attr, None, class_map,
            "L5_stale_prune", 2,
            target_is_unknown=True,
        ),
        _v15_7a_build_episode(
            [f"The {distractors[1]} is {rng.choice(HOLDOUT_STATES)}."],
            entity, attr, None, class_map,
            "L5_stale_prune", 3,
            target_is_unknown=True,
        ),
        _v15_7a_build_episode(
            [f"The {distractors[2]} is in the {rng.choice(HOLDOUT_LOCATIONS)}."],
            entity, attr, None, class_map,
            "L5_stale_prune", 4,
            target_is_unknown=True,
        ),
        _v15_7a_build_episode(
            [f"The {distractors[3]} is {rng.choice(HOLDOUT_COLORS)}."],
            entity, attr, None, class_map,
            "L5_stale_prune", 5,
            target_is_unknown=True,
        ),
    ...
    return LongitudinalSequence(
        sequence_id=f"L5_{entity}_stale",
        family_tag="L5_stale_prune",
        primary_entity_id=entity,
        episodes=episodes,
        expected_final_committed={},
        expected_provisional_present=[],
        expected_provisional_absent=[
            (entity, attr, val1_idx),
            (entity, attr, val2_idx),
        ],
        expected_ops={"RECONCILE": 1, "PRUNE": 2},
        description=(
            "ep1 conflict creates provisional {val1:{ep1}, val2:{ep1}}. "
            "ep2-ep4: no activity on slot. At end_ep4, inactivity=3 → "
            "both entries pruned. ep5 is post-prune verification."
        ),
    )


LONGITUDINAL_FAMILIES = {
    "L1_promote_cycle":   gen_L1_promote_cycle,
    "L2_retrograde_only": gen_L2_retrograde_only,
    "L3_completion":      gen_L3_completion,
    "L4_no_inflation":    gen_L4_no_inflation,
    "L5_stale_prune":     gen_L5_stale_prune,
}


# Pas 7a config (used in later sections; placed here so the regime is
# self-contained as a module)
V15_7A_PAS7A_PARAMS = {
    "N_promote":       2,
    "M_retrograde":    2,
    "K_promote_age":   2,
...
print("[v15.7a Pas 7a] Section C1: LongitudinalEpisodeRegime defined")
print(f"        - LongitudinalSequence dataclass (11 fields)")
print(f"        - {len(LONGITUDINAL_FAMILIES)} generators: "
      f"{list(LONGITUDINAL_FAMILIES.keys())}")
print(f"        - params: N_promote={V15_7A_PAS7A_PARAMS['N_promote']}  "
      f"M_retrograde={V15_7A_PAS7A_PARAMS['M_retrograde']}  "
      f"K_promote_age={V15_7A_PAS7A_PARAMS['K_promote_age']}  "
      f"K_prune_stale={V15_7A_PAS7A_PARAMS['K_prune_stale']}")
print(f"        - no consolidator yet; sequences are data only (ready for D.2 smoke)")


# ===========================================================================
# v15.7a PAS 7A — Section C2: Baseline sequence runner (D.2)
# ===========================================================================
#
# Runs each L1-L5 sequence through the Pas 6 arbiter WITHOUT consolidator,
# capturing final (bank, provisional) state and comparing to predicted state.
#
# Purpose: establish empirical baseline for what Pas 6 alone produces on
# longitudinal sequences. The delta vs predicted (which assumes Pas 7a
# consolidator IS active) defines exactly what D.3-D.8 closes.
#
# State persistence rule for a sequence:
#   - Bank, ProvisionalMemory, EpisodeBuffer, StabilityIndex persist
#     ACROSS episodes within one sequence (no reset between eps).
#   - ALL state is reset BETWEEN sequences.
#
# No consolidator runs here. Still uses CommitArbiterPas6 (RoMR + Pas 3
# entity span composer stay active — Pas 7a builds on exactly that base).
# ===========================================================================


@dataclass
class V15_7a_SequenceState:
    """State captured at the end of a sequence, for comparison vs predicted."""
    bank_snapshot:          Dict[Tuple[str, str], int]           # (ent, attr) -> value_idx
    provisional_entries:    List[Tuple[str, str, int, int]]      # (ent, attr, value, ep_id)
    consolidator_ops_count: Dict[str, int]                        # always 0 in D.2


@dataclass
class V15_7a_BaselineTrial:
    """Single baseline trial: sequence run + observed vs expected."""
    sequence_id:       str
    family_tag:        str
    expected:          "LongitudinalSequence"
    observed:          V15_7a_SequenceState
    committed_match:   bool
    provisional_match: bool
    ops_match:         bool


def _v15_7a_snapshot_sequence_state(bank, provisional_memory,
                                        consolidator_ops_count: Dict[str, int]
                                        ) -> V15_7a_SequenceState:
    """Capture current state as a SequenceState."""
    bank_snap: Dict[Tuple[str, str], int] = {}
    for slot_id in bank.occupied_slots():
        rec = bank.get_record(slot_id)
        ent = rec.entity_id
        for attr_type, slot_obj in rec.attr_slots.items():
            if slot_obj is None or not slot_obj.present:
                continue
            bank_snap[(ent, attr_type)] = slot_obj.value_idx

    prov_entries: List[Tuple[str, str, int, int]] = []
    for e in provisional_memory.entries:
        prov_entries.append((e.entity_id, e.attr_type, e.value_idx, e.episode_id))

    return V15_7a_SequenceState(
        bank_snapshot=bank_snap,
        provisional_entries=prov_entries,
        consolidator_ops_count=dict(consolidator_ops_count),
    )


def _v15_7a_compare_state_vs_expected(observed: V15_7a_SequenceState,
                                          expected: "LongitudinalSequence"
                                          ) -> Tuple[bool, bool, bool]:
    """Return (committed_match, provisional_match, ops_match).

    Semantics:
      - committed_match: observed bank snapshot EQUALS predicted committed.
      - provisional_match: expected_present is a SUBSET of observed, AND
        expected_absent has ZERO intersection with observed.
      - ops_match: observed consolidator op counts EQUAL predicted.
    """
    committed_ok = (observed.bank_snapshot == expected.expected_final_committed)

    observed_prov_set = {(e, a, v) for (e, a, v, _) in observed.provisional_entries}
    expected_present = set(expected.expected_provisional_present)
    expected_absent  = set(expected.expected_provisional_absent)
    provisional_ok = (expected_present.issubset(observed_prov_set)
                        and not (expected_absent & observed_prov_set))

    ops_ok = (observed.consolidator_ops_count == expected.expected_ops)

    return committed_ok, provisional_ok, ops_ok


def v15_7a_run_baseline_d2(base_model, v15_1_memory,
                               n_per_family: int = 20,
                               seed: int = 20261102) -> Dict:
    """D.2 smoke test: baseline sequence measurement WITHOUT consolidator.

    Establishes what Pas 6 produces on longitudinal sequences. The delta
    vs expected (which assumes consolidator IS active) is exactly what
    D.3-D.8 closes.
    """
    print()
    print(SEP)
    print(f"[v15.7a Pas 7a D.2] BASELINE SEQUENCE MEASUREMENT (no consolidator)")
    print(f"  n_per_family = {n_per_family}, seed = {seed}")
    print(SEP)
    ...
    # Fresh state for D.2 run (does NOT touch Pas 6 globals)
    bank = DeterministicObjectBank(capacity=64,
                                       d_model=base_model.config.hidden_dim)
    provisional_memory = ProvisionalMemory()
    episode_buffer     = EpisodeBuffer()
    stability_index    = BankStabilityIndex()
    arbiter = CommitArbiterPas6(bank, provisional_memory, episode_buffer,
                                    stability_index,
                                    composer_trace_log=None,
                                    romr_trace_log=None)

    results_per_family: Dict[str, List[V15_7a_BaselineTrial]] = {}
    # distinct episode_id range from Pas 6 evaluator to avoid collision
    episode_counter = 7_000_000

    for family_name, gen in LONGITUDINAL_FAMILIES.items():
        print(f"\n  Running family {family_name} (n={n_per_family})")
        rng = _rng_module.Random(seed + hash(family_name) % 10000)
        trials: List[V15_7a_BaselineTrial] = []

        for trial_idx in range(n_per_family):
            ...
            seq = gen(rng, ENC, class_map)

            # Run each episode; state persists between eps in the sequence
            for ep in seq.episodes:
                arbiter.begin_episode(episode_counter)
                for j, fact_text in enumerate(ep.facts):
                    arbiter.write_fact(fact_text, ent_fn, cls_fn, val_fn,
                                          write_step=j)
                arbiter.end_episode(ent_fn, cls_fn, val_fn)
                episode_counter += 1

            observed = _v15_7a_snapshot_sequence_state(
                bank, provisional_memory,
                consolidator_ops_count={}
            )
            committed_ok, provisional_ok, ops_ok = (
                _v15_7a_compare_state_vs_expected(observed, seq)
            )
            trials.append(V15_7a_BaselineTrial(
                sequence_id=seq.sequence_id,
                family_tag=seq.family_tag,
                expected=seq,
                observed=observed,
                committed_match=committed_ok,
                provisional_match=provisional_ok,
                ops_match=ops_ok,
            ))

        results_per_family[family_name] = trials

        n_committed_match = sum(1 for t in trials if t.committed_match)
        n_provisional_match = sum(1 for t in trials if t.provisional_match)
        n_ops_match = sum(1 for t in trials if t.ops_match)
        print(f"    committed_match:   {n_committed_match}/{len(trials)}")
        print(f"    provisional_match: {n_provisional_match}/{len(trials)}")
        print(f"    ops_match:         {n_ops_match}/{len(trials)}")

        if trials:
            t = trials[0]
            print(f"    --- first trial detail ({t.sequence_id}) ---")
            print(f"    expected committed:    {t.expected.expected_final_committed}")
            print(f"    observed committed:    {t.observed.bank_snapshot}")
            print(f"    expected prov present: {t.expected.expected_provisional_present}")
            print(f"    observed prov entries: "
                  f"{t.observed.provisional_entries[:5]}"
                  f"{'...' if len(t.observed.provisional_entries) > 5 else ''}")
            print(f"    expected ops:          {t.expected.expected_ops}")
    ...
    print(SEP)
    print("L3 (completion):  predictions should MATCH entirely (consolidator is no-op).")
    print("L4 (no_inflation): committed + provisional match; ops diverge (no RECONCILE run).")
    print("L1/L2/L5:          predictions DIVERGE by design — these are the scenarios the")
    print("                   consolidator must close in D.3-D.8.")

    roll_up: Dict[str, Dict] = {}
    for family, trials in results_per_family.items():
        n = len(trials)
        roll_up[family] = {
            "n": n,
            "committed_match_rate":   sum(1 for t in trials if t.committed_match) / max(1, n),
            "provisional_match_rate": sum(1 for t in trials if t.provisional_match) / max(1, n),
            "ops_match_rate":         sum(1 for t in trials if t.ops_match) / max(1, n),
        }

    print()
    print(f"  {'family':25s}  {'committed':>10s} {'provisional':>12s} {'ops':>8s}")
    for family, r in roll_up.items():
        print(f"  {family:25s}  {r['committed_match_rate']:>10.3f} "
              f"{r['provisional_match_rate']:>12.3f} {r['ops_match_rate']:>8.3f}")

    return {
        "config":      {"n_per_family": n_per_family, "seed": seed},
        "roll_up":     roll_up,
        "per_family":  {
            f: [{
                "sequence_id":           t.sequence_id,
                "family_tag":            t.family_tag,
                "committed_match":       t.committed_match,
                "provisional_match":     t.provisional_match,
                "ops_match":             t.ops_match,
                "expected_committed":    dict(t.expected.expected_final_committed),
                "observed_committed":    dict(t.observed.bank_snapshot),
                "expected_ops":          dict(t.expected.expected_ops),
                "n_observed_provisional": len(t.observed.provisional_entries),
            } for t in trials]
            for f, trials in results_per_family.items()
        },
    }


print("[v15.7a Pas 7a] Section C2: D.2 baseline runner defined")
print("        - runs L1-L5 sequences through CommitArbiterPas6 (NO consolidator)")
print("        - state persists across episodes within a sequence")
print("        - reports committed/provisional/ops match vs predicted")
print("        - activate via env V15_7A_D2_MODE=run (defaults to skip)")


# ---- Dispatch (gated, defaults to skip) ------------------------------------
V15_7A_D2_MODE = os.environ.get("V15_7A_D2_MODE", "skip")
if V15_7A_D2_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.2] Running baseline (V15_7A_D2_MODE=run)")
    print(SEP)
    d2_result = v15_7a_run_baseline_d2(base_model_v15_6, v15_1_memory_v15_6,
                                            n_per_family=20)
    V15_7A_DIR         = os.path.join(PROJECT_ROOT, "v15_7a")
    V15_7A_RESULTS_DIR = os.path.join(V15_7A_DIR, "results")
    os.makedirs(V15_7A_RESULTS_DIR, exist_ok=True)
    d2_path = os.path.join(V15_7A_RESULTS_DIR, "v15_7a_d2_baseline.json")
    with open(d2_path, "w") as f:
        json.dump(d2_result, f, indent=2, default=str)
    print(f"\n[v15.7a Pas 7a D.2] saved: {d2_path}")


# ===========================================================================
# v15.7a PAS 7A — Section C3: ProvisionalEntry derivation layer (D.3)
# ===========================================================================
#
# Pure functions over List[ProvisionalEntry]: confirmation_episodes,
# last_activity_episode, first_seen_episode + 3 consolidator predicates.
# No class mutation — the Pas 2 dataclass at line 11175 stays byte-identical.
# ===========================================================================


def _v15_7a_confirmation_episodes(entries: List["ProvisionalEntry"],
                                      entity_id: str, attr_type: str,
                                      value_idx: int) -> Set[int]:
    """Distinct episode_ids where (slot, value_idx) appeared in provisional."""
    return {e.episode_id for e in entries
            if e.entity_id == entity_id and e.attr_type == attr_type
            and e.value_idx == value_idx}


def _v15_7a_last_activity_episode(entries: List["ProvisionalEntry"],
                                      entity_id: str, attr_type: str
                                      ) -> Optional[int]:
    """Max episode_id across all values for this slot, or None."""
    eps = [e.episode_id for e in entries
           if e.entity_id == entity_id and e.attr_type == attr_type]
    return max(eps) if eps else None


def _v15_7a_first_seen_episode(entries: List["ProvisionalEntry"],
                                   entity_id: str, attr_type: str,
                                   value_idx: int) -> Optional[int]:
    """Min episode_id for (slot, value_idx), or None."""
    eps = [e.episode_id for e in entries
           if e.entity_id == entity_id and e.attr_type == attr_type
           and e.value_idx == value_idx]
...
def _v15_7a_distinct_values_for_slot(entries: List["ProvisionalEntry"],
                                          entity_id: str, attr_type: str
                                          ) -> Set[int]:
    """Distinct value_idx observed in provisional for this slot."""
    return {e.value_idx for e in entries
            if e.entity_id == entity_id and e.attr_type == attr_type}


# Consolidator params (README C3):
V15_7A_N_PROMOTE       = 2    # distinct episodes for promote
V15_7A_M_RETROGRADE    = 2    # distinct challenger episodes for retrograde
V15_7A_K_PROMOTE_AGE   = 2    # min episodes between first_seen and promote
V15_7A_K_PRUNE_STALE   = 3    # episodes of silence before prune


def _v15_7a_is_promote_eligible(entries: List["ProvisionalEntry"],
                                    entity_id: str, attr_type: str,
                                    value_idx: int,
                                    current_episode: int,
                                    N: int = V15_7A_N_PROMOTE,
                                    K_age: int = V15_7A_K_PROMOTE_AGE
                                    ) -> bool:
    """|confirms| >= N AND (current_ep - first_seen) >= K_age."""
    confirms = _v15_7a_confirmation_episodes(entries, entity_id, attr_type,
                                                 value_idx)
    if len(confirms) < N:
        return False
    first = _v15_7a_first_seen_episode(entries, entity_id, attr_type,
                                           value_idx)
    if first is None:
        return False
    return (current_episode - first) >= K_age


def _v15_7a_is_retrograde_eligible(entries: List["ProvisionalEntry"],
                                        entity_id: str, attr_type: str,
                                        committed_value_idx: int,
                                        M: int = V15_7A_M_RETROGRADE
                                        ) -> Tuple[bool, Optional[int]]:
    """Strongest challenger (value_idx != committed) with |confirms| >= M.

    Tiebreak: largest count, then smallest value_idx (determinism for D.8).
    """
    candidates: List[Tuple[int, int, int]] = []  # (neg_count, value_idx, n)
    for v in _v15_7a_distinct_values_for_slot(entries, entity_id, attr_type):
        if v == committed_value_idx:
            continue
        confirms = _v15_7a_confirmation_episodes(entries, entity_id,
                                                     attr_type, v)
        if len(confirms) >= M:
            candidates.append((-len(confirms), v, len(confirms)))
    if not candidates:
        return (False, None)
    candidates.sort()
...
def _v15_7a_is_stale_for_prune(entries: List["ProvisionalEntry"],
                                    entity_id: str, attr_type: str,
                                    current_episode: int,
                                    K_stale: int = V15_7A_K_PRUNE_STALE
                                    ) -> bool:
    """(current_ep - last_activity) >= K_stale, False on absent slot."""
    last = _v15_7a_last_activity_episode(entries, entity_id, attr_type)
    if last is None:
        return False
    return (current_episode - last) >= K_stale


print("[v15.7a Pas 7a] Section C3: ProvisionalEntry derivation layer ready")
print("        - _v15_7a_confirmation_episodes / _last_activity_episode"
      " / _first_seen_episode / _distinct_values_for_slot")
print("        - predicates: is_promote_eligible / is_retrograde_eligible"
      " / is_stale_for_prune")
print(f"        - params: N={V15_7A_N_PROMOTE} M={V15_7A_M_RETROGRADE}"
      f" K_age={V15_7A_K_PROMOTE_AGE} K_stale={V15_7A_K_PRUNE_STALE}")


# ---- Dispatch (gated, defaults to skip) ------------------------------------
V15_7A_D3_MODE = os.environ.get("V15_7A_D3_MODE", "skip")
if V15_7A_D3_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.3] Running derivation self-check"
          " (V15_7A_D3_MODE=run)")
    print(SEP)

    def _mk(ent: str, attr: str, val: int, ep: int) -> "ProvisionalEntry":
        return ProvisionalEntry(
            entity_id=ent, attr_type=attr, value_idx=val,
            episode_id=ep, write_step=0, source_text="",
        )
    ...
    def _assert(name: str, cond: bool, detail: str = ""):
        if cond:
            ...
            msg = f"{name} FAILED {detail}".strip()
            failures.append(msg)
            print(f"        [FAIL] {msg}")

    # Test 1: confirmation_episodes dedupes within episode
    es = [_mk("e1", "color", 5, 10), _mk("e1", "color", 5, 10),
          _mk("e1", "color", 5, 11)]
    got = _v15_7a_confirmation_episodes(es, "e1", "color", 5)
    _assert("T1 dedupe within episode",
            got == {10, 11},
            f"expected {{10,11}} got {got}")

    # Test 2: confirmation_episodes isolates by value_idx
    es = [_mk("e1", "color", 5, 10), _mk("e1", "color", 7, 11),
          _mk("e1", "color", 5, 12)]
    got = _v15_7a_confirmation_episodes(es, "e1", "color", 5)
    _assert("T2 isolate by value",
            got == {10, 12},
            f"expected {{10,12}} got {got}")

    # Test 3: last_activity_episode across all values of slot
    es = [_mk("e1", "color", 5, 10), _mk("e1", "color", 7, 13),
          _mk("e2", "color", 5, 99)]
    got = _v15_7a_last_activity_episode(es, "e1", "color")
    _assert("T3 last_activity_episode",
            got == 13,
            f"expected 13 got {got}")

    # Test 4: last_activity_episode returns None on absent slot
    got = _v15_7a_last_activity_episode([], "ex", "color")
    _assert("T4 last_activity on empty", got is None)

    # Test 5: first_seen_episode
    es = [_mk("e1", "color", 5, 12), _mk("e1", "color", 5, 10),
          _mk("e1", "color", 5, 11)]
    got = _v15_7a_first_seen_episode(es, "e1", "color", 5)
    _assert("T5 first_seen_episode", got == 10, f"got {got}")

    # Test 6: is_promote_eligible — N satisfied, age satisfied
    es = [_mk("e1", "color", 5, 10), _mk("e1", "color", 5, 11)]
    got = _v15_7a_is_promote_eligible(es, "e1", "color", 5,
                                           current_episode=12)
    _assert("T6 promote eligible @N=2 age=2", got is True)

    # Test 7: is_promote_eligible — N insufficient
    es = [_mk("e1", "color", 5, 10)]
    got = _v15_7a_is_promote_eligible(es, "e1", "color", 5,
                                           current_episode=99)
    _assert("T7 promote blocked @N<2", got is False)

    # Test 8: is_promote_eligible — same-episode writes = 1 confirmation
    es = [_mk("e1", "color", 5, 10), _mk("e1", "color", 5, 10),
          _mk("e1", "color", 5, 10)]
    got = _v15_7a_is_promote_eligible(es, "e1", "color", 5,
                                           current_episode=99)
    _assert("T8 anti-inflation (L4 scenario)", got is False,
            "intra-episode repetition must not count as N=2")

    # Test 9: is_promote_eligible — age not yet satisfied
    es = [_mk("e1", "color", 5, 10), _mk("e1", "color", 5, 11)]
    got = _v15_7a_is_promote_eligible(es, "e1", "color", 5,
                                           current_episode=11)
    _assert("T9 promote blocked @age<K_age",
            got is False,
            "current_ep - first_seen = 1 < K_age=2")

    # Test 10: is_retrograde_eligible — challenger with M=2 episodes wins
    es = [_mk("e1", "color", 9, 11), _mk("e1", "color", 9, 12)]
    ok, chal = _v15_7a_is_retrograde_eligible(es, "e1", "color",
                                                   committed_value_idx=5)
    _assert("T10 retrograde with M=2 challenger",
            ok is True and chal == 9,
            f"got ({ok}, {chal})")

    # Test 11: is_retrograde_eligible — committed's own value ignored
    es = [_mk("e1", "color", 5, 11), _mk("e1", "color", 5, 12)]
    ok, chal = _v15_7a_is_retrograde_eligible(es, "e1", "color",
                                                   committed_value_idx=5)
    _assert("T11 retrograde ignores committed value",
            ok is False and chal is None,
            f"got ({ok}, {chal})")

    # Test 12: is_retrograde_eligible — picks strongest challenger
    es = [_mk("e1", "color", 7, 11),                              # 1 ep
          _mk("e1", "color", 9, 11), _mk("e1", "color", 9, 12)]   # 2 ep
    ok, chal = _v15_7a_is_retrograde_eligible(es, "e1", "color",
                                                   committed_value_idx=5)
    _assert("T12 retrograde picks strongest",
            ok is True and chal == 9,
            f"got ({ok}, {chal})")

    # Test 13: is_stale_for_prune — K_stale=3 hit
    es = [_mk("e1", "color", 5, 10)]
    _assert("T13a stale @current=13 K=3",
            _v15_7a_is_stale_for_prune(es, "e1", "color", 13) is True)
    _assert("T13b not stale @current=12 K=3",
            _v15_7a_is_stale_for_prune(es, "e1", "color", 12) is False)

    # Test 14: stale on absent slot = False (no-op guarantee)
    _assert("T14 stale on empty",
            _v15_7a_is_stale_for_prune([], "e1", "color", 99) is False)

    print()
    if failures:
        print(f"[v15.7a Pas 7a D.3] FAIL: {len(failures)} self-check(s)"
              f" failed")
        for m in failures:
            print(f"        - {m}")
    else:
        print("[v15.7a Pas 7a D.3] PASS: all 15 derivation self-checks"
              " green")


# ===========================================================================
# v15.7a PAS 7A — Section C4: Consolidator.reconcile (D.4)
# ===========================================================================
#
# RECONCILE fires per (slot, end_episode) where the slot received >=1 new
# entry THIS episode AND the post-add total is >=2. Collapses exact
# (slot, value, episode_id) duplicates; emits 1 ConsolidationRecord per fire.
# Mutates ProvisionalMemory.entries in-place. Does NOT touch bank.
# ===========================================================================
...
class V15_7a_ConsolidationRecord:
    """Audit trail entry for one consolidator op (README C2)."""
    episode_id:   int
    operation:    str   # RECONCILE | PRUNE | RETROGRADE | PROMOTE
    entity_id:    str
    attr_type:    str
...
def _v15_7a_reconcile(provisional_memory: "ProvisionalMemory",
                          current_episode: int,
                          audit: List[V15_7a_ConsolidationRecord]
                          ) -> int:
    """For each slot touched this episode with >=2 total entries, collapse
    exact (value_idx, episode_id) duplicates. Returns op count."""
    slots_touched: Set[Tuple[str, str]] = {
        (e.entity_id, e.attr_type)
        for e in provisional_memory.entries
        if e.episode_id == current_episode
    }
    op_count = 0
    for (ent, attr) in sorted(slots_touched):    # sorted for determinism
        slot_entries = [e for e in provisional_memory.entries
                        if e.entity_id == ent and e.attr_type == attr]
        if len(slot_entries) < 2:
            continue

        seen: Dict[Tuple[int, int], "ProvisionalEntry"] = {}
        for e in slot_entries:
            key = (e.value_idx, e.episode_id)
            if key not in seen or e.write_step < seen[key].write_step:
                seen[key] = e
        canonical = list(seen.values())
        before_count = len(slot_entries)
        after_count = len(canonical)

        provisional_memory.entries = (
            [e for e in provisional_memory.entries
             if not (e.entity_id == ent and e.attr_type == attr)]
            + canonical
        )

        audit.append(V15_7a_ConsolidationRecord(
            episode_id=current_episode,
            operation="RECONCILE",
            entity_id=ent,
            attr_type=attr,
            value_idx=None,
            reason=f"slot has {before_count} entries; canonical={after_count}",
            state_before={"entries_count": before_count},
            state_after={"entries_count": after_count},
        ))
        op_count += 1
    return op_count


print("[v15.7a Pas 7a] Section C4: Consolidator.reconcile + audit ready")


# ---- Dispatch (gated, defaults to skip) ------------------------------------
V15_7A_D4_MODE = os.environ.get("V15_7A_D4_MODE", "skip")
if V15_7A_D4_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.4] Running reconcile self-check"
          " (V15_7A_D4_MODE=run)")
    print(SEP)

    def _mk(ent: str, attr: str, val: int, ep: int,
                ws: int = 0) -> "ProvisionalEntry":
        return ProvisionalEntry(
            entity_id=ent, attr_type=attr, value_idx=val,
            episode_id=ep, write_step=ws, source_text="",
        )

    failures4: List[str] = []

    def _assert4(name: str, cond: bool, detail: str = ""):
        if cond:
            print(f"        [OK]   {name}")
        else:
            msg = f"{name} FAILED {detail}".strip()
            failures4.append(msg)
            print(f"        [FAIL] {msg}")

    # Test 1: L4 ep1 — (val1,ep1) x2, (val2,ep1) x1 -> collapse to 2 entries
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1, ws=0),
                  _mk("e1", "color", 5, 1, ws=1),
                  _mk("e1", "color", 7, 1, ws=2)]
    log: List[V15_7a_ConsolidationRecord] = []
    n = _v15_7a_reconcile(pm, current_episode=1, audit=log)
    _assert4("T1 L4-ep1: op_count=1",          n == 1)
    _assert4("T1 L4-ep1: entries collapsed 3->2",
             len(pm.entries) == 2,
             f"got {len(pm.entries)}")
    _assert4("T1 L4-ep1: distinct (val,ep) pairs preserved",
             {(e.value_idx, e.episode_id) for e in pm.entries}
             == {(5, 1), (7, 1)})
    _assert4("T1 L4-ep1: audit logged 1 RECONCILE",
             len(log) == 1 and log[0].operation == "RECONCILE")

    # Test 2: L5 ep1 — (val1,ep1), (val2,ep1) -> no collapse, op fires
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1), _mk("e1", "color", 7, 1)]
    log = []
    n = _v15_7a_reconcile(pm, current_episode=1, audit=log)
    _assert4("T2 L5-ep1: op_count=1 (audit event)", n == 1)
    _assert4("T2 L5-ep1: entries unchanged 2->2",  len(pm.entries) == 2)

    # Test 3: L1 ep3 — (blue,ep2), (blue,ep3) — touched-this-ep + total>=2
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n = _v15_7a_reconcile(pm, current_episode=3, audit=log)
    _assert4("T3 L1-ep3: op_count=1", n == 1)
    _assert4("T3 L1-ep3: entries unchanged 2->2",  len(pm.entries) == 2)

    # Test 4: L1 ep4 — slot NOT touched this ep -> no fire
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n = _v15_7a_reconcile(pm, current_episode=4, audit=log)
    _assert4("T4 L1-ep4 (slot not touched): op_count=0", n == 0)
    _assert4("T4 L1-ep4: audit empty",  len(log) == 0)

    # Test 5: L1 ep2 — single new entry, total<2 -> no fire
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2)]
    log = []
    n = _v15_7a_reconcile(pm, current_episode=2, audit=log)
    _assert4("T5 L1-ep2 (only 1 entry): op_count=0", n == 0)

    # Test 6: L3 — different slots per episode, no fire ever
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1), _mk("e1", "size", 3, 2)]
    log = []
    n = _v15_7a_reconcile(pm, current_episode=2, audit=log)
    _assert4("T6 L3: cross-slot no-op", n == 0)

    # Test 7: idempotency — second run finds nothing to do
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1, ws=0),
                  _mk("e1", "color", 5, 1, ws=1)]
    log = []
    _v15_7a_reconcile(pm, current_episode=1, audit=log)
    n2 = _v15_7a_reconcile(pm, current_episode=1, audit=log)
    _assert4("T7 idempotency: second call op_count=0", n2 == 0)
    _assert4("T7 idempotency: entries stay collapsed",
             len(pm.entries) == 1)

    # Test 8: write_step tiebreak — earliest write_step survives in canonical
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1, ws=7),
                  _mk("e1", "color", 5, 1, ws=2),
                  _mk("e1", "color", 5, 1, ws=5)]
    log = []
    _v15_7a_reconcile(pm, current_episode=1, audit=log)
    _assert4("T8 tiebreak: kept entry has min write_step",
             len(pm.entries) == 1 and pm.entries[0].write_step == 2)

    # Test 9: bank untouched — reconcile MUST NOT mutate bank
    # (we don't have a bank stub here; verify reconcile signature accepts
    #  only provisional_memory + episode + audit, no bank arg)
    import inspect
    sig = inspect.signature(_v15_7a_reconcile)
    _assert4("T9 reconcile signature has no bank parameter",
             "bank" not in sig.parameters,
             f"params: {list(sig.parameters)}")

    # Test 10: cross-entity isolation — only this-episode-touched slots fire
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1), _mk("e1", "color", 7, 1),
                  _mk("e2", "color", 5, 0), _mk("e2", "color", 7, 0)]
    log = []
    n = _v15_7a_reconcile(pm, current_episode=1, audit=log)
    _assert4("T10 cross-entity: only e1 fires (touched ep=1)", n == 1)
    _assert4("T10 cross-entity: e2 entries untouched",
             sum(1 for e in pm.entries if e.entity_id == "e2") == 2)

    print()
    if failures4:
        print(f"[v15.7a Pas 7a D.4] FAIL: {len(failures4)} self-check(s)"
              f" failed")
        for m in failures4:
            print(f"        - {m}")
    else:
        print("[v15.7a Pas 7a D.4] PASS: all 18 reconcile self-checks"
              " green")


# ===========================================================================
# v15.7a PAS 7A — Section C5: Consolidator.prune (D.5)
# ===========================================================================
#
# PRUNE fires per ENTRY satisfying _v15_7a_is_stale_for_prune at end_episode.
# 1 op count per entry pruned (matches L5 expected PRUNE: 2 for 2 entries).
# Mutates ProvisionalMemory.entries in-place. Does NOT touch bank.
# Order of operations (README C3): reconcile → prune → retrograde → promote.
# ===========================================================================


def _v15_7a_prune(provisional_memory: "ProvisionalMemory",
                      current_episode: int,
                      audit: List[V15_7a_ConsolidationRecord],
                      K_stale: int = V15_7A_K_PRUNE_STALE
                      ) -> int:
    """For each entry whose slot is_stale_for_prune at current_episode,
    drop the entry. Returns number of entries pruned."""
    op_count = 0
    # Distinct slots present in provisional
    slots: Set[Tuple[str, str]] = {
        (e.entity_id, e.attr_type) for e in provisional_memory.entries
    }
    for (ent, attr) in sorted(slots):    # sorted for determinism
        if not _v15_7a_is_stale_for_prune(provisional_memory.entries,
                                              ent, attr, current_episode,
                                              K_stale):
            continue

        # Snapshot entries before mutation for audit
        slot_entries = [e for e in provisional_memory.entries
                        if e.entity_id == ent and e.attr_type == attr]
        before_count = len(slot_entries)
        last_act = _v15_7a_last_activity_episode(provisional_memory.entries,
                                                     ent, attr)

        # Drop in deterministic order: by (value_idx, episode_id, write_step)
        slot_entries.sort(key=lambda e: (e.value_idx, e.episode_id,
                                            e.write_step))
        for e in slot_entries:
            audit.append(V15_7a_ConsolidationRecord(
                episode_id=current_episode,
                ...
                entity_id=ent,
                attr_type=attr,
                value_idx=e.value_idx,
                reason=(f"last_activity={last_act} current={current_episode}"
                        f" diff={current_episode - last_act} >= K={K_stale}"),
                state_before={"entries_count_for_slot": before_count,
                              "value_idx": e.value_idx,
                              "episode_id": e.episode_id},
                state_after={"entries_count_for_slot": 0},
            ))
            op_count += 1

        # Remove all slot entries in one pass
        provisional_memory.entries = [
            e for e in provisional_memory.entries
            if not (e.entity_id == ent and e.attr_type == attr)
        ]
...
print("[v15.7a Pas 7a] Section C5: Consolidator.prune ready")


# ---- Dispatch (gated, defaults to skip) ------------------------------------
V15_7A_D5_MODE = os.environ.get("V15_7A_D5_MODE", "skip")
if V15_7A_D5_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.5] Running prune self-check"
          " (V15_7A_D5_MODE=run)")
    print(SEP)
    ...
    def _assert5(name: str, cond: bool, detail: str = ""):
        if cond:
            print(f"        [OK]   {name}")
        else:
            msg = f"{name} FAILED {detail}".strip()
            failures5.append(msg)
            print(f"        [FAIL] {msg}")

    # T1: L5 end_ep4 — 2 entries (val1,ep1) (val2,ep1), K=3, diff=3 → both prune
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1), _mk("e1", "color", 7, 1)]
    log: List[V15_7a_ConsolidationRecord] = []
    n = _v15_7a_prune(pm, current_episode=4, audit=log)
    _assert5("T1 L5-ep4: op_count=2 (per-entry)", n == 2,
             f"got {n}")
    _assert5("T1 L5-ep4: entries pruned 2->0",   len(pm.entries) == 0)
    _assert5("T1 L5-ep4: audit logs 2 PRUNE",
             len(log) == 2 and all(r.operation == "PRUNE" for r in log))

    # T2: L4 end_ep3 — 2 entries (val1,ep1) (val2,ep1), K=3, diff=2 → no prune
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1), _mk("e1", "color", 7, 1)]
    log = []
    n = _v15_7a_prune(pm, current_episode=3, audit=log)
    _assert5("T2 L4-ep3 (diff=2 < K=3): op_count=0", n == 0)
    _assert5("T2 L4-ep3: entries unchanged 2->2",   len(pm.entries) == 2)

    # T3: edge — diff exactly equals K → prune (>= semantics from is_stale_for_prune)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1)]
    log = []
    n = _v15_7a_prune(pm, current_episode=4, audit=log)
    _assert5("T3 boundary diff==K=3: prunes", n == 1)

    # T4: diff = K-1 → no prune
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1)]
    log = []
    n = _v15_7a_prune(pm, current_episode=3, audit=log)
    _assert5("T4 boundary diff==K-1: no prune", n == 0)

    # T5: cross-slot — only stale slots pruned, fresh slots untouched
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1),     # stale @ep5
                  _mk("e1", "size",  3, 4)]      # fresh @ep5 (diff=1)
    log = []
    n = _v15_7a_prune(pm, current_episode=5, audit=log)
    _assert5("T5 cross-slot: only color pruned", n == 1)
    _assert5("T5 cross-slot: size entry untouched",
             len(pm.entries) == 1 and pm.entries[0].attr_type == "size")

    # T6: empty memory → no-op
    pm = ProvisionalMemory()
    pm.entries = []
    log = []
    n = _v15_7a_prune(pm, current_episode=99, audit=log)
    _assert5("T6 empty memory: op_count=0", n == 0)

    # T7: signature has no bank parameter (must NOT mutate bank)
    import inspect
    sig = inspect.signature(_v15_7a_prune)
    _assert5("T7 prune signature has no bank parameter",
             "bank" not in sig.parameters,
             f"params: {list(sig.parameters)}")

    # T8: idempotency — second call after full prune is no-op
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1), _mk("e1", "color", 7, 1)]
    log = []
    _v15_7a_prune(pm, current_episode=4, audit=log)
    n2 = _v15_7a_prune(pm, current_episode=4, audit=log)
    _assert5("T8 idempotency: second call op_count=0", n2 == 0)

    # T9: cross-entity — slot e1 stale, slot e2 fresh
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1),    # diff=4
                  _mk("e2", "color", 7, 4)]     # diff=1
    log = []
    n = _v15_7a_prune(pm, current_episode=5, audit=log)
    _assert5("T9 cross-entity: only e1 pruned", n == 1)
    _assert5("T9 cross-entity: e2 untouched",
             len(pm.entries) == 1 and pm.entries[0].entity_id == "e2")

    # T10: audit reason references diff and K
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1)]
    log = []
    _v15_7a_prune(pm, current_episode=5, audit=log)
    _assert5("T10 audit reason mentions diff",
             "diff=4" in log[0].reason and "K=3" in log[0].reason,
             f"got reason: {log[0].reason}")

    # T11: deterministic order within slot (sorted by value_idx)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 1), _mk("e1", "color", 5, 1),
                  _mk("e1", "color", 7, 1)]
    log = []
    _v15_7a_prune(pm, current_episode=5, audit=log)
    pruned_values = [r.value_idx for r in log]
    _assert5("T11 deterministic prune order: ascending value_idx",
             pruned_values == [5, 7, 9],
             f"got {pruned_values}")

    print()
    if failures5:
        print(f"[v15.7a Pas 7a D.5] FAIL: {len(failures5)} self-check(s)"
              f" failed")
        for m in failures5:
            print(f"        - {m}")
    else:
        print("[v15.7a Pas 7a D.5] PASS: all 16 prune self-checks"
              " green")


# ===========================================================================
# v15.7a PAS 7A — Section D6: Consolidator.retrograde
# ===========================================================================
#
# RETROGRADE fires per committed (entity_id, attr_type) slot whose
# is_retrograde_eligible predicate holds against the current ProvisionalMemory:
# a strongest challenger value (value_idx != committed) with >= M distinct
# episode_ids in provisional. Demotes the bank attribute slot
# (present=False, value_idx=-1, version+=1, value_emb=None) and removes the
# entry from BankStabilityIndex. Provisional entries are NOT modified in v1
# (the challenger remains, allowing D7 promote to elevate it in a later
# end_episode; same-end_episode promote is blocked by intra-pas exclusion).
#
# Counting: 1 op per (slot, end_episode) demoted (matches L1/L2 expected
# RETROGRADE: 1). Mutates bank + stability_index. Audit trail required.
#
# Invariants (README C4):
#   - one-slot locality: only same (entity_id, attr_type)
#   - no cross-entity bleed
#   - no cross-slot reasoning
#   - atomic per slot
#   - no cooldown in v1
#   - challenger committed-value identity is excluded by is_retrograde_eligible
#
# Order of operations (README C3): reconcile -> prune -> retrograde -> promote.
# ===========================================================================


def _v15_7a_retrograde(provisional_memory: "ProvisionalMemory",
                           bank: "DeterministicObjectBank",
                           stability_index: "BankStabilityIndex",
                           current_episode: int,
                           audit: List[V15_7a_ConsolidationRecord],
                           M: int = V15_7A_M_RETROGRADE) -> int:
    """For each committed (entity_id, attr_type) slot, evaluate
    is_retrograde_eligible against provisional entries. If eligible, demote
    the bank slot in-place and remove the slot from stability_index.

    Returns op_count = number of slots demoted this end_episode.
    """
    op_count = 0
    # Iterate stability_index's committed slots in deterministic order.
    # Snapshot keys first because we mutate committed_episode in-loop.
    committed_slots = sorted(stability_index.committed_episode.keys())

    for (ent, attr) in committed_slots:
        # Look up current committed value from bank.
        bank_slot = bank.find_by_entity_id(ent)
        if bank_slot is None:
            continue    # bank has no record for this entity (defensive)
        rec = bank.get_record(bank_slot)
        if rec is None:
            continue
        attr_slot = rec.attr_slots.get(attr)
        if attr_slot is None or not attr_slot.present:
            continue    # slot already cleared (defensive; preserves no-op
                        # guarantee on missing committed state)
        committed_value_idx = attr_slot.value_idx

        # Eligibility check against provisional. is_retrograde_eligible
        # excludes value_idx == committed_value_idx by construction.
        eligible, challenger_value_idx = _v15_7a_is_retrograde_eligible(
            provisional_memory.entries, ent, attr, committed_value_idx, M=M
        )
        if not eligible:
            continue

        # Snapshot for audit BEFORE mutation.
        committed_episode_was = stability_index.episode_of(ent, attr)
        version_before = attr_slot.version
        challenger_confirms = _v15_7a_confirmation_episodes(
            provisional_memory.entries, ent, attr, challenger_value_idx
        )

        # Demote in-place. write_step preserved (no new write occurred);
        # version bumped so any downstream consumer can detect the change.
        # value_emb cleared so no stale shadow embedding lingers on a
        # not-present slot.
        attr_slot.present   = False
        attr_slot.value_idx = -1
        attr_slot.version   = version_before + 1
        attr_slot.value_emb = None

        # Remove slot from stability index — slot is no longer stable.
        del stability_index.committed_episode[(ent, attr)]
        ...
        audit.append(V15_7a_ConsolidationRecord(
            ...
            entity_id=ent,
            attr_type=attr,
            value_idx=committed_value_idx,
            reason=(f"committed_value={committed_value_idx} demoted; "
                    f"challenger={challenger_value_idx} with "
                    f"{len(challenger_confirms)} distinct episodes "
                    f">= M={M}"),
            state_before={
                "bank_present":                True,
                "bank_value_idx":              committed_value_idx,
                "bank_version":                version_before,
                "stability_committed_episode": committed_episode_was,
            },
            state_after={
                "bank_present":                False,
                "bank_value_idx":              -1,
                "bank_version":                version_before + 1,
                "stability_committed_episode": None,
            },
        ))
        op_count += 1
...
print("[v15.7a Pas 7a] Section D6: Consolidator.retrograde ready")


# ---- Dispatch (gated, defaults to skip) ------------------------------------
V15_7A_D6_MODE = os.environ.get("V15_7A_D6_MODE", "skip")
if V15_7A_D6_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.6] Running retrograde self-check"
          " (V15_7A_D6_MODE=run) — Gate 4 focus: false_retrograde_rate=0")
    print(SEP)

    # Local stubs that mimic the bank + stability index interface used by
    # _v15_7a_retrograde. Mutation patterns mirror DeterministicObjectBank
    # and BankStabilityIndex from Section A / Section P2A.
    ...
    @dataclass
    class _D6_ObjectRecord:
        entity_id:  str
        attr_slots: Dict[str, _D6_AttrSlot] = field(default_factory=dict)

    class _D6_Bank:
        def __init__(self):
            self._records: Dict[int, _D6_ObjectRecord] = {}
            self._entity_id_to_slot: Dict[str, int] = {}
            self._next_slot = 0

        def find_by_entity_id(self, ent: str) -> Optional[int]:
            return self._entity_id_to_slot.get(ent)

        def get_record(self, slot: int) -> Optional[_D6_ObjectRecord]:
            return self._records.get(slot)

        def commit_slot(self, ent: str, attr: str, value_idx: int,
                            version: int = 0, write_step: int = 0):
            slot = self._entity_id_to_slot.get(ent)
            if slot is None:
                slot = self._next_slot
                self._next_slot += 1
                self._records[slot] = _D6_ObjectRecord(entity_id=ent)
                self._entity_id_to_slot[ent] = slot
            self._records[slot].attr_slots[attr] = _D6_AttrSlot(
                present=True, value_idx=value_idx, version=version,
                write_step=write_step,
            )

    class _D6_StabilityIndex:
        def __init__(self):
            self.committed_episode: Dict[Tuple[str, str], int] = {}

        def mark_committed(self, ent: str, attr: str, ep: int):
            self.committed_episode[(ent, attr)] = ep

        def episode_of(self, ent: str, attr: str) -> Optional[int]:
            return self.committed_episode.get((ent, attr))
    ...
    def _assert6(name: str, cond: bool, detail: str = ""):
        if cond:
            print(f"        [OK]   {name}")
        else:
            msg = f"{name} FAILED {detail}".strip()
            failures6.append(msg)
            print(f"        [FAIL] {msg}")

    # T1: L2 happy path — committed red, 2 challenger episodes blue
    bank = _D6_Bank();  bank.commit_slot("e1", "color", 5, version=1)
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log: List[V15_7a_ConsolidationRecord] = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T1 L2: op_count=1", n == 1, f"got {n}")
    rec = bank.get_record(0)
    _assert6("T1 L2: bank slot demoted (present=False)",
             rec.attr_slots["color"].present is False)
    _assert6("T1 L2: bank value cleared (value_idx=-1)",
             rec.attr_slots["color"].value_idx == -1)
    _assert6("T1 L2: bank version bumped 1->2",
             rec.attr_slots["color"].version == 2)
    _assert6("T1 L2: bank value_emb cleared",
             rec.attr_slots["color"].value_emb is None)
    _assert6("T1 L2: stability index entry removed",
             ("e1", "color") not in si.committed_episode)
    _assert6("T1 L2: provisional entries NOT modified (challenger stays)",
             len(pm.entries) == 2)
    _assert6("T1 L2: audit logs 1 RETROGRADE",
             len(log) == 1 and log[0].operation == "RETROGRADE")
    _assert6("T1 L2: audit value_idx = old committed (5)",
             log[0].value_idx == 5, f"got {log[0].value_idx}")
    _assert6("T1 L2: audit reason names challenger=9 and M",
             "challenger=9" in log[0].reason and "M=2" in log[0].reason,
             f"got: {log[0].reason}")

    # T2: L3 completion — separate slots committed, no challengers
    # Gate 4 critical: ZERO false retrogrades on completions.
    bank = _D6_Bank()
    bank.commit_slot("e1", "color",    5, version=1, write_step=0)
    bank.commit_slot("e1", "size",     3, version=1, write_step=1)
    bank.commit_slot("e1", "location", 7, version=1, write_step=2)
    si = _D6_StabilityIndex()
    si.mark_committed("e1", "color", 1)
    si.mark_committed("e1", "size", 2)
    si.mark_committed("e1", "location", 3)
    pm = ProvisionalMemory()    # empty: completion has no provisional
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=4, audit=log)
    _assert6("T2 L3: op_count=0 (no challengers anywhere)", n == 0)
    _assert6("T2 L3: all 3 stability entries preserved",
             len(si.committed_episode) == 3)
    _assert6("T2 L3: bank color still committed",
             bank.get_record(0).attr_slots["color"].present is True
             and bank.get_record(0).attr_slots["color"].value_idx == 5)
    _assert6("T2 L3: bank size still committed",
             bank.get_record(0).attr_slots["size"].present is True
             and bank.get_record(0).attr_slots["size"].value_idx == 3)
    _assert6("T2 L3: bank location still committed",
             bank.get_record(0).attr_slots["location"].present is True
             and bank.get_record(0).attr_slots["location"].value_idx == 7)
    _assert6("T2 L3: empty audit", len(log) == 0)

    # T3: L4 anti-inflation — same-episode dups, distinct=1 < M=2
    bank = _D6_Bank();  bank.commit_slot("e1", "color", 5, version=1)
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2, ws=0),
                  _mk("e1", "color", 9, 2, ws=1),
                  _mk("e1", "color", 9, 2, ws=2)]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=2, audit=log)
    _assert6("T3 L4 anti-inflation: op_count=0 (1 distinct ep < M=2)",
             n == 0)
    _assert6("T3 L4: bank intact",
             bank.get_record(0).attr_slots["color"].present is True
             and bank.get_record(0).attr_slots["color"].value_idx == 5)
    _assert6("T3 L4: stability intact",
             ("e1", "color") in si.committed_episode)

    # T4: L5 stale provisional only, nothing committed
    bank = _D6_Bank()    # no committed slots
    si = _D6_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 1), _mk("e1", "color", 7, 1)]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=4, audit=log)
    _assert6("T4 L5: op_count=0 (nothing committed)", n == 0)
    _assert6("T4 L5: empty audit", len(log) == 0)

    # T5: insufficient challengers — 1 episode only
    bank = _D6_Bank();  bank.commit_slot("e1", "color", 5, version=1)
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2)]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=2, audit=log)
    _assert6("T5 insufficient: op_count=0 (1 challenger ep < M=2)", n == 0)
    _assert6("T5 insufficient: bank intact",
             bank.get_record(0).attr_slots["color"].present is True)

    # T6: challenger value == committed value — must NOT trigger retrograde
    # (re-confirmations of the committed value are not challengers)
    bank = _D6_Bank();  bank.commit_slot("e1", "color", 5, version=1)
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 5, 2), _mk("e1", "color", 5, 3)]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T6 self-confirmation: op_count=0 (no challenger != committed)",
             n == 0)
    _assert6("T6 self-confirmation: bank intact",
             bank.get_record(0).attr_slots["color"].present is True
             and bank.get_record(0).attr_slots["color"].value_idx == 5)

    # T7: cross-entity isolation — only e1 has eligible challenger
    bank = _D6_Bank()
    bank.commit_slot("e1", "color", 5, version=1)
    bank.commit_slot("e2", "color", 5, version=1)
    si = _D6_StabilityIndex()
    si.mark_committed("e1", "color", 1)
    si.mark_committed("e2", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T7 cross-entity: op_count=1 (only e1)", n == 1)
    _assert6("T7 cross-entity: e1 demoted",
             bank.get_record(bank.find_by_entity_id("e1"))
                 .attr_slots["color"].present is False)
    _assert6("T7 cross-entity: e2 untouched",
             bank.get_record(bank.find_by_entity_id("e2"))
                 .attr_slots["color"].present is True
             and bank.get_record(bank.find_by_entity_id("e2"))
                 .attr_slots["color"].value_idx == 5)
    _assert6("T7 cross-entity: e2 stability preserved",
             ("e2", "color") in si.committed_episode)

    # T8: cross-slot isolation — only color demoted, size untouched
    bank = _D6_Bank()
    bank.commit_slot("e1", "color", 5, version=1)
    bank.commit_slot("e1", "size",  3, version=1)
    si = _D6_StabilityIndex()
    si.mark_committed("e1", "color", 1)
    si.mark_committed("e1", "size",  1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T8 cross-slot: op_count=1 (color only)", n == 1)
    _assert6("T8 cross-slot: color demoted",
             bank.get_record(0).attr_slots["color"].present is False)
    _assert6("T8 cross-slot: size untouched",
             bank.get_record(0).attr_slots["size"].present is True
             and bank.get_record(0).attr_slots["size"].value_idx == 3)
    _assert6("T8 cross-slot: size stability preserved",
             ("e1", "size") in si.committed_episode)

    # T9: idempotency — second call does nothing (slot removed from stability)
    bank = _D6_Bank();  bank.commit_slot("e1", "color", 5, version=1)
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n1 = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    n2 = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T9 idempotency: first call op_count=1", n1 == 1)
    _assert6("T9 idempotency: second call op_count=0", n2 == 0)
    _assert6("T9 idempotency: audit count stays 1", len(log) == 1)

    # T10: determinism — sorted iteration order over committed slots
    bank = _D6_Bank()
    bank.commit_slot("z_e", "color", 5, version=1)
    bank.commit_slot("a_e", "color", 5, version=1)
    bank.commit_slot("m_e", "color", 5, version=1)
    si = _D6_StabilityIndex()
    si.mark_committed("z_e", "color", 1)
    si.mark_committed("a_e", "color", 1)
    si.mark_committed("m_e", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [
        _mk("z_e", "color", 9, 2), _mk("z_e", "color", 9, 3),
        _mk("a_e", "color", 9, 2), _mk("a_e", "color", 9, 3),
        _mk("m_e", "color", 9, 2), _mk("m_e", "color", 9, 3),
    ]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T10 determinism: 3 ops fired", n == 3)
    _assert6("T10 determinism: audit order = ascending entity_id",
             [r.entity_id for r in log] == ["a_e", "m_e", "z_e"],
             f"got {[r.entity_id for r in log]}")

    # T11: signature must require bank + stability_index params
    # (D6 is the first op that mutates bank — explicit in signature)
    import inspect
    sig = inspect.signature(_v15_7a_retrograde)
    _assert6("T11 signature has bank parameter",
             "bank" in sig.parameters)
    _assert6("T11 signature has stability_index parameter",
             "stability_index" in sig.parameters)

    # T12: defensive — committed slot in stability but missing in bank
    # (should be a no-op for that slot, never raise)
    bank = _D6_Bank()    # bank empty
    si = _D6_StabilityIndex();  si.mark_committed("ghost", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("ghost", "color", 9, 2),
                  _mk("ghost", "color", 9, 3)]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T12 defensive: missing-from-bank = silent skip",
             n == 0 and len(log) == 0)
    _assert6("T12 defensive: stability entry still present "
             "(D6 doesn't manage strays)",
             ("ghost", "color") in si.committed_episode)

    # T13: defensive — stability lists slot, bank record exists but
    # attr_slot.present is already False (was demoted by something else)
    bank = _D6_Bank()
    bank.commit_slot("e1", "color", 5, version=1)
    # Manually clear (simulates external demote)
    bank.get_record(0).attr_slots["color"].present = False
    bank.get_record(0).attr_slots["color"].value_idx = -1
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T13 defensive: not-present slot = no-op", n == 0)

    # T14: picks STRONGEST challenger when multiple eligible
    bank = _D6_Bank();  bank.commit_slot("e1", "color", 5, version=1)
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [
        _mk("e1", "color", 7, 2), _mk("e1", "color", 7, 3),       # 2 eps
        _mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3),
        _mk("e1", "color", 9, 4),                                  # 3 eps
    ]
    log = []
    n = _v15_7a_retrograde(pm, bank, si, current_episode=4, audit=log)
    _assert6("T14 strongest: op_count=1", n == 1)
    _assert6("T14 strongest: audit names challenger=9 (3 eps)",
             "challenger=9" in log[0].reason
             and "3 distinct" in log[0].reason,
             f"got: {log[0].reason}")

    # T15: audit state_before/state_after schema
    bank = _D6_Bank();  bank.commit_slot("e1", "color", 5, version=2)
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    rec = log[0]
    _assert6("T15 audit state_before.bank_present=True",
             rec.state_before["bank_present"] is True)
    _assert6("T15 audit state_before.bank_value_idx=5",
             rec.state_before["bank_value_idx"] == 5)
    _assert6("T15 audit state_before.bank_version=2",
             rec.state_before["bank_version"] == 2)
    _assert6("T15 audit state_before.stability_committed_episode=1",
             rec.state_before["stability_committed_episode"] == 1)
    _assert6("T15 audit state_after.bank_present=False",
             rec.state_after["bank_present"] is False)
    _assert6("T15 audit state_after.bank_version=3 (bumped)",
             rec.state_after["bank_version"] == 3)
    _assert6("T15 audit state_after.stability_committed_episode=None",
             rec.state_after["stability_committed_episode"] is None)
    _assert6("T15 audit episode_id = current_episode (3)",
             rec.episode_id == 3)

    # T16: write_step preserved on demote (no new write)
    bank = _D6_Bank()
    bank.commit_slot("e1", "color", 5, version=1, write_step=42)
    si = _D6_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    _v15_7a_retrograde(pm, bank, si, current_episode=3, audit=log)
    _assert6("T16 write_step preserved (42) on demote",
             bank.get_record(0).attr_slots["color"].write_step == 42)

    # T17: GATE 4 INTEGRATION — across L1..L5 scenarios, false_retrograde=0
    # A "false retrograde" is one fired on a slot that should have stayed.
    # L3 (T2) and L4 (T3) and L5 (T4) and T5 and T6 are all false-retrograde
    # negative-cases — counted across the suite, total false retrogrades = 0
    # iff each of those tests reported op_count==0.
    print()
    print("        [GATE 4 ROLLUP] false_retrograde_rate target = 0")
    print(f"        L3 completion (T2):        op_count must be 0")
    print(f"        L4 anti-inflation (T3):    op_count must be 0")
    print(f"        L5 stale only (T4):        op_count must be 0")
    print(f"        Insufficient challengers (T5): op_count must be 0")
    print(f"        Self-confirmation (T6):    op_count must be 0")
    # If any of T2..T6 failed, failures6 already contains the failure.
    g4_negative_failures = [m for m in failures6
                              if any(k in m for k in
                                     ["T2 L3", "T3 L4", "T4 L5",
                                      "T5 insufficient",
                                      "T6 self-confirmation"])]
    _assert6("T17 GATE 4: false_retrograde_rate = 0 across negatives",
             len(g4_negative_failures) == 0,
             f"violations: {g4_negative_failures}")

    print()
    if failures6:
        print(f"[v15.7a Pas 7a D.6] FAIL: {len(failures6)} self-check(s)"
              f" failed")
        for m in failures6:
            print(f"        - {m}")
        print()
        print("        GATE 4 STATUS: BLOCKED — D.7 promote MUST NOT proceed")
    else:
        print("[v15.7a Pas 7a D.6] PASS: all retrograde self-checks green")
        print()
        print("        GATE 4 STATUS: PASS — false_retrograde_rate=0 confirmed")
        print("        D.7 promote may proceed.")


# ===========================================================================
# v15.7a PAS 7A — Section D7: Consolidator.promote
# ===========================================================================
#
# PROMOTE fires per (entity_id, attr_type) slot in ProvisionalMemory whose
# strongest candidate value satisfies is_promote_eligible (N distinct
# confirmation episodes AND age >= K_age) AND is not blocked by intra-pas
# exclusion AND the bank is in a state that allows promotion.
#
# Intra-pas exclusion (README C4 #1): a slot that received a RETROGRADE op in
# THIS end_episode (audit scan) is skipped. Forces the L1 1-episode delay.
#
# Bank-state policy (one-slot locality, no transitive demote):
#   - bank entity exists, slot.present == False (or attr absent):
#       -> promote: write value, mark stability, clear provisional (slot,val)
#   - bank entity exists, slot.present == True with SAME value_idx:
#       -> mark stability if missing, clear provisional (slot,val), audit
#   - bank entity exists, slot.present == True with DIFFERENT value:
#       -> SKIP (D6 didn't demote it earlier this episode -> not eligible).
#          Promote does NOT silently overwrite stable bank values.
#   - bank entity DOES NOT exist:
#       -> SKIP in v1 (no entity_emb available; consolidator is bank-mutator
#          only on already-allocated entities).
#
# Counting: 1 op per slot promoted (matches L1 expected PROMOTE: 1).
# Mutates bank + stability_index + provisional_memory. Audit trail required.
#
# Order of operations (README C3):
#     reconcile -> prune -> retrograde -> PROMOTE.
# ===========================================================================


def _v15_7a_promote(provisional_memory: "ProvisionalMemory",
                        bank: "DeterministicObjectBank",
                        stability_index: "BankStabilityIndex",
                        current_episode: int,
                        audit: List[V15_7a_ConsolidationRecord],
                        N: int = V15_7A_N_PROMOTE,
                        K_age: int = V15_7A_K_PROMOTE_AGE) -> int:
    """For each provisional slot, evaluate is_promote_eligible. If eligible
    AND not blocked by intra-pas RETROGRADE AND bank state permits, promote
    the value to bank, mark stability_index, and clear provisional entries
    for (slot, value).

    Returns op_count = number of slots promoted this end_episode.
    """
    # Intra-pas exclusion: any slot retrograded in this end_episode.
    excluded_slots: Set[Tuple[str, str]] = {
        (r.entity_id, r.attr_type) for r in audit
        if r.operation == "RETROGRADE" and r.episode_id == current_episode
    }

    # Distinct slots present in provisional, deterministic order.
    slots: Set[Tuple[str, str]] = {
        (e.entity_id, e.attr_type) for e in provisional_memory.entries
    }
    op_count = 0

    for (ent, attr) in sorted(slots):
        if (ent, attr) in excluded_slots:
            continue    # blocked by intra-pas exclusion

        # Find eligible candidate values; pick the strongest deterministically.
        # Tiebreak: most confirmations, then smallest value_idx.
        candidates: List[Tuple[int, int, int]] = []
        # (neg_confirms, value_idx, n_confirms)
        for v in _v15_7a_distinct_values_for_slot(provisional_memory.entries,
                                                       ent, attr):
            if not _v15_7a_is_promote_eligible(provisional_memory.entries,
                                                    ent, attr, v,
                                                    current_episode=current_episode,
                                                    N=N, K_age=K_age):
                continue
            confirms = _v15_7a_confirmation_episodes(
                provisional_memory.entries, ent, attr, v
            )
            candidates.append((-len(confirms), v, len(confirms)))
        ...
        if not candidates:
            continue
        candidates.sort()
        promote_value_idx = candidates[0][1]
        promote_n_confirms = candidates[0][2]
        first_seen = _v15_7a_first_seen_episode(provisional_memory.entries,
                                                     ent, attr,
                                                     promote_value_idx)
        ...
        if bank_slot is None:
            audit.append(V15_7a_ConsolidationRecord(
                episode_id=current_episode,
                operation="PROMOTE_SKIPPED",
                entity_id=ent,
                attr_type=attr,
                value_idx=promote_value_idx,
                reason=("entity not in bank; v1 consolidator does not "
                        "allocate"),
                state_before={"bank_entity_present": False},
                state_after={"bank_entity_present": False},
            ))
            continue
        ...
        # Case A: bank slot is empty/absent -> promote
        # Case B: bank slot has same value -> idempotent finalize
        # Case C: bank slot has DIFFERENT value -> skip (no silent overwrite)
        if attr_slot is not None and attr_slot.present \
                and attr_slot.value_idx != promote_value_idx:
            audit.append(V15_7a_ConsolidationRecord(
                episode_id=current_episode,
                operation="PROMOTE_SKIPPED",
                entity_id=ent,
                attr_type=attr,
                value_idx=promote_value_idx,
                reason=(f"bank stable at value_idx="
                        f"{attr_slot.value_idx}; promote would overwrite, "
                        f"v1 forbids transitive demote"),
                state_before={
                    "bank_present":   True,
                    "bank_value_idx": attr_slot.value_idx,
                    "bank_version":   attr_slot.version,
                },
                state_after={
                    "bank_present":   True,
                    "bank_value_idx": attr_slot.value_idx,
        ...
        if attr_slot is None:
            ...
            present_before    = attr_slot.present
            value_idx_before  = attr_slot.value_idx
            version_before    = attr_slot.version
            write_step_before = attr_slot.write_step
        stability_before = stability_index.episode_of(ent, attr)

        # Promote: in-place mutation (mirrors D6 style; avoids needing
        # an entity_emb_fn / value_emb_fn for the consolidator).
        if attr_slot is None:
            # Defensive: bank entity exists but attr_slot missing entirely.
            # Should not happen with current __post_init__ on ObjectRecord
            # (creates default slots for color/size/location/state), but
            # safe-fallback: skip; consolidator does not allocate slots.
            audit.append(V15_7a_ConsolidationRecord(
                episode_id=current_episode,
                operation="PROMOTE_SKIPPED",
                entity_id=ent,
                attr_type=attr,
                value_idx=promote_value_idx,
                reason="attr_slot absent on existing entity",
                state_before={"attr_slot_present": False},
                state_after={"attr_slot_present": False},
            ))
            continue

        attr_slot.present   = True
        attr_slot.value_idx = promote_value_idx
        attr_slot.version   = version_before + 1
        # write_step: -1 sentinel = consolidator-promoted, no in-episode write
        attr_slot.write_step = -1
        # value_emb left as-is (None after a prior D6 demote, or whatever
        # was there). Consolidator does not regenerate embeddings.

        stability_index.committed_episode[(ent, attr)] = current_episode

        # Clear provisional entries for (slot, promoted_value). Other values
        # in provisional for the same slot are NOT removed — they remain as
        # historical challengers.
        before_prov_count = sum(
            1 for e in provisional_memory.entries
            if e.entity_id == ent and e.attr_type == attr
            and e.value_idx == promote_value_idx
        )
        provisional_memory.entries = [
            e for e in provisional_memory.entries
            if not (e.entity_id == ent and e.attr_type == attr
        ...
        audit.append(V15_7a_ConsolidationRecord(
            ...
            attr_type=attr,
            value_idx=promote_value_idx,
            reason=(f"value={promote_value_idx} promoted: "
                    f"{promote_n_confirms} distinct episodes >= N={N}; "
                    f"first_seen={first_seen} current={current_episode} "
                    f"age={current_episode - first_seen} >= K_age={K_age}"),
            state_before={
                "bank_present":                present_before,
                "bank_value_idx":              value_idx_before,
                "bank_version":                version_before,
                "stability_committed_episode": stability_before,
                "provisional_entries_for_value": before_prov_count,
            },
            state_after={
                "bank_present":                True,
                "bank_value_idx":              promote_value_idx,
                "bank_version":                version_before + 1,
                "stability_committed_episode": current_episode,
                "provisional_entries_for_value": 0,
            },
        ))
        op_count += 1
...
print("[v15.7a Pas 7a] Section D7: Consolidator.promote ready")


# ---- Dispatch (gated, defaults to skip) ------------------------------------
V15_7A_D7_MODE = os.environ.get("V15_7A_D7_MODE", "skip")
if V15_7A_D7_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.7] Running promote self-check"
          " (V15_7A_D7_MODE=run) — Gate 3 focus: false_promote_rate=0")
    print(SEP)
    ...
    @dataclass
    class _D7_ObjectRecord:
        entity_id:  str
        attr_slots: Dict[str, _D7_AttrSlot] = field(default_factory=dict)

    class _D7_Bank:
        def __init__(self):
            self._records: Dict[int, _D7_ObjectRecord] = {}
            self._entity_id_to_slot: Dict[str, int] = {}
            self._next_slot = 0
        ...
        def get_record(self, slot: int) -> Optional[_D7_ObjectRecord]:
            return self._records.get(slot)

        def allocate_entity(self, ent: str,
                                with_slots: Optional[List[str]] = None):
            slot = self._entity_id_to_slot.get(ent)
            if slot is None:
                slot = self._next_slot
                self._next_slot += 1
                rec = _D7_ObjectRecord(entity_id=ent)
                if with_slots:
                    for a in with_slots:
                        rec.attr_slots[a] = _D7_AttrSlot()
                self._records[slot] = rec
                self._entity_id_to_slot[ent] = slot
            return slot

        def commit_slot(self, ent: str, attr: str, value_idx: int,
                            version: int = 1, write_step: int = 0):
            self.allocate_entity(ent, with_slots=[attr])
            slot = self._entity_id_to_slot[ent]
            self._records[slot].attr_slots[attr] = _D7_AttrSlot(
                present=True, value_idx=value_idx, version=version,
                write_step=write_step,
            )

        def empty_slot(self, ent: str, attr: str, version: int = 0):
            self.allocate_entity(ent, with_slots=[attr])
            slot = self._entity_id_to_slot[ent]
            self._records[slot].attr_slots[attr] = _D7_AttrSlot(
                present=False, value_idx=-1, version=version,
            )

    class _D7_StabilityIndex:
        def __init__(self):
            self.committed_episode: Dict[Tuple[str, str], int] = {}
    ...
    def _assert7(name: str, cond: bool, detail: str = ""):
        if cond:
            print(f"        [OK]   {name}")
        else:
            msg = f"{name} FAILED {detail}".strip()
            failures7.append(msg)
            print(f"        [FAIL] {msg}")

    # T1: L1 end_ep4 happy path — bank empty (post-D6), provisional has blue
    # @ep2 and ep3, current=4. Eligible: N=2, age=2.
    bank = _D7_Bank();  bank.empty_slot("e1", "color", version=2)
    si = _D7_StabilityIndex()    # cleared by D6 in this scenario
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log: List[V15_7a_ConsolidationRecord] = []
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T1 L1-ep4: op_count=1", n == 1, f"got {n}")
    rec = bank.get_record(0)
    _assert7("T1 L1-ep4: bank slot promoted (present=True)",
             rec.attr_slots["color"].present is True)
    _assert7("T1 L1-ep4: bank value_idx = 9 (blue)",
             rec.attr_slots["color"].value_idx == 9)
    _assert7("T1 L1-ep4: bank version bumped 2->3",
             rec.attr_slots["color"].version == 3)
    _assert7("T1 L1-ep4: bank write_step = -1 (consolidator sentinel)",
             rec.attr_slots["color"].write_step == -1)
    _assert7("T1 L1-ep4: stability_index marks committed_episode = 4",
             si.episode_of("e1", "color") == 4)
    _assert7("T1 L1-ep4: provisional cleared for promoted (slot, value)",
             len(pm.entries) == 0)
    _assert7("T1 L1-ep4: audit logs 1 PROMOTE",
             len(log) == 1 and log[0].operation == "PROMOTE")
    _assert7("T1 L1-ep4: audit value_idx = 9",
             log[0].value_idx == 9)
    _assert7("T1 L1-ep4: audit reason names confirmations and age",
             "2 distinct" in log[0].reason
             and "age=2" in log[0].reason
             and "N=2" in log[0].reason,
             f"got: {log[0].reason}")

    # T2: INTRA-PAS EXCLUSION — slot retrograded same end_episode is skipped
    # Setup: pre-existing audit log already contains a RETROGRADE for (e1,color)
    # at episode 3. Then promote runs at end_ep3 with eligible blue.
    bank = _D7_Bank();  bank.empty_slot("e1", "color", version=2)
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 1), _mk("e1", "color", 9, 3)]
    log = [V15_7a_ConsolidationRecord(
        episode_id=3, operation="RETROGRADE",
        entity_id="e1", attr_type="color", value_idx=5,
        reason="prior d6", state_before={}, state_after={},
    )]
    n = _v15_7a_promote(pm, bank, si, current_episode=3, audit=log)
    _assert7("T2 intra-pas exclusion: op_count=0", n == 0)
    _assert7("T2 intra-pas exclusion: bank stays empty",
             bank.get_record(0).attr_slots["color"].present is False)
    _assert7("T2 intra-pas exclusion: provisional NOT cleared",
             len(pm.entries) == 2)
    _assert7("T2 intra-pas exclusion: stability NOT marked",
             si.episode_of("e1", "color") is None)

    # T3: intra-pas exclusion is per-end_episode — retrograde from a PRIOR
    # episode does NOT block promote in current episode
    bank = _D7_Bank();  bank.empty_slot("e1", "color", version=2)
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = [V15_7a_ConsolidationRecord(
        episode_id=3, operation="RETROGRADE",    # PRIOR end_episode
        entity_id="e1", attr_type="color", value_idx=5,
        reason="prior d6", state_before={}, state_after={},
    )]
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T3 prior-episode retrograde does NOT block current promote",
             n == 1)

    # T4: GATE 3 — N insufficient (1 distinct ep) -> NO promote
    bank = _D7_Bank();  bank.empty_slot("e1", "color", version=2)
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2)]
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=99, audit=log)
    _assert7("T4 GATE 3: insufficient confirmations -> op_count=0", n == 0)
    _assert7("T4 GATE 3: bank stays empty",
             bank.get_record(0).attr_slots["color"].present is False)

    # T5: GATE 3 — L4 anti-inflation: 3 same-episode dups = 1 distinct ep
    bank = _D7_Bank();  bank.empty_slot("e1", "color")
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2, ws=0),
                  _mk("e1", "color", 9, 2, ws=1),
    ...
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=99, audit=log)
    _assert7("T5 GATE 3 L4: same-ep dups = 1 distinct -> op_count=0", n == 0)

    # T6: age not yet satisfied -> blocked
    bank = _D7_Bank();  bank.empty_slot("e1", "color")
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    # current=3, first_seen=2, age=1 < K_age=2 -> NOT eligible
    n = _v15_7a_promote(pm, bank, si, current_episode=3, audit=log)
    _assert7("T6 age<K_age: op_count=0", n == 0)
    # current=4, first_seen=2, age=2 -> eligible
    n2 = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T6 age=K_age: op_count=1", n2 == 1)

    # T7: GATE 3 — bank stable with DIFFERENT value -> SKIP (no silent overwrite)
    bank = _D7_Bank();  bank.commit_slot("e1", "color", 5, version=2)
    si = _D7_StabilityIndex();  si.mark_committed("e1", "color", 1)
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T7 GATE 3: stable bank diff value -> op_count=0", n == 0)
    _assert7("T7 GATE 3: bank value preserved (5)",
             bank.get_record(0).attr_slots["color"].value_idx == 5)
    _assert7("T7 GATE 3: bank version preserved (2)",
             bank.get_record(0).attr_slots["color"].version == 2)
    _assert7("T7 GATE 3: stability preserved",
             si.episode_of("e1", "color") == 1)
    _assert7("T7 GATE 3: provisional NOT cleared (challenger remains)",
             len(pm.entries) == 2)
    _assert7("T7 GATE 3: audit logs PROMOTE_SKIPPED",
             len(log) == 1 and log[0].operation == "PROMOTE_SKIPPED")

    # T8: bank entity DOES NOT exist -> SKIP (v1 consolidator no-allocate)
    bank = _D7_Bank()    # e1 not allocated
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T8 entity not in bank: op_count=0", n == 0)
    _assert7("T8 entity not in bank: provisional NOT cleared",
             len(pm.entries) == 2)
    _assert7("T8 entity not in bank: audit logs PROMOTE_SKIPPED",
             len(log) == 1 and log[0].operation == "PROMOTE_SKIPPED"
             and "entity not in bank" in log[0].reason)

    # T9: bank stable with SAME value -> idempotent finalize
    # Bank already has blue committed; provisional carries the same value
    # with N+age satisfied. Provisional gets cleared, stability re-marked
    # to current episode, audit logs PROMOTE.
    bank = _D7_Bank();  bank.commit_slot("e1", "color", 9, version=1)
    si = _D7_StabilityIndex()    # not yet in stability_index
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T9 idempotent same-value: op_count=1", n == 1)
    _assert7("T9 idempotent: bank value still 9",
             bank.get_record(0).attr_slots["color"].value_idx == 9)
    _assert7("T9 idempotent: stability now marked at current (4)",
             si.episode_of("e1", "color") == 4)
    _assert7("T9 idempotent: provisional cleared",
             len(pm.entries) == 0)

    # T10: cross-entity isolation — only e1 promotes
    bank = _D7_Bank()
    bank.empty_slot("e1", "color", version=1)
    bank.empty_slot("e2", "color", version=1)
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [
        _mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3),
        _mk("e2", "color", 7, 2),                            # only 1 ep
    ]
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T10 cross-entity: op_count=1 (only e1 eligible)", n == 1)
    _assert7("T10 cross-entity: e1 promoted to 9",
             bank.get_record(bank.find_by_entity_id("e1"))
                 .attr_slots["color"].value_idx == 9)
    _assert7("T10 cross-entity: e2 stays empty",
             bank.get_record(bank.find_by_entity_id("e2"))
                 .attr_slots["color"].present is False)
    _assert7("T10 cross-entity: e2 provisional remains",
             any(e.entity_id == "e2" for e in pm.entries))

    # T11: cross-slot isolation — color promotes, size insufficient
    bank = _D7_Bank()
    bank.empty_slot("e1", "color", version=1)
    bank.empty_slot("e1", "size",  version=1)
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [
        _mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3),
        _mk("e1", "size",  3, 2),                            # only 1 ep
    ]
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T11 cross-slot: op_count=1 (color only)", n == 1)
    _assert7("T11 cross-slot: color promoted",
             bank.get_record(0).attr_slots["color"].value_idx == 9)
    _assert7("T11 cross-slot: size stays empty",
             bank.get_record(0).attr_slots["size"].present is False)
    _assert7("T11 cross-slot: size stability NOT marked",
             si.episode_of("e1", "size") is None)

    # T12: idempotency — second call after promote = 0 ops
    # First call clears the (slot, promoted_value) provisional entries, so
    # the second call has no candidates and is a no-op.
    bank = _D7_Bank();  bank.empty_slot("e1", "color")
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    n1 = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    n2 = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T12 idempotency: first call op_count=1", n1 == 1)
    _assert7("T12 idempotency: second call op_count=0 "
             "(provisional emptied; no eligible candidates left)",
             n2 == 0, f"got {n2}")
    _assert7("T12 idempotency: audit count stays at 1 PROMOTE",
             sum(1 for r in log if r.operation == "PROMOTE") == 1)
    _assert7("T12 idempotency: bank value remains 9",
             bank.get_record(0).attr_slots["color"].value_idx == 9)

    # T13: determinism — sorted iteration order over slots
    bank = _D7_Bank()
    bank.empty_slot("z_e", "color", version=1)
    bank.empty_slot("a_e", "color", version=1)
    bank.empty_slot("m_e", "color", version=1)
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [
        _mk("z_e", "color", 9, 2), _mk("z_e", "color", 9, 3),
    ...
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T13 determinism: 3 ops fired", n == 3)
    promote_records = [r for r in log if r.operation == "PROMOTE"]
    _assert7("T13 determinism: audit order = ascending entity_id",
             [r.entity_id for r in promote_records]
             == ["a_e", "m_e", "z_e"],
             f"got {[r.entity_id for r in promote_records]}")

    # T14: signature requires bank + stability_index
    import inspect
    sig = inspect.signature(_v15_7a_promote)
    _assert7("T14 signature has bank parameter",
             "bank" in sig.parameters)
    _assert7("T14 signature has stability_index parameter",
             "stability_index" in sig.parameters)

    # T15: tiebreak — strongest challenger wins (most confirmations,
    # then smallest value_idx)
    bank = _D7_Bank();  bank.empty_slot("e1", "color")
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [
        _mk("e1", "color", 7, 1), _mk("e1", "color", 7, 2),       # 2 eps
        _mk("e1", "color", 9, 1), _mk("e1", "color", 9, 2),
        _mk("e1", "color", 9, 3),                                  # 3 eps
    ]
    log = []
    n = _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    _assert7("T15 tiebreak: op_count=1 (one slot, strongest value)", n == 1)
    _assert7("T15 tiebreak: promoted value=9 (3 eps beats 7's 2 eps)",
             bank.get_record(0).attr_slots["color"].value_idx == 9)
    _assert7("T15 tiebreak: only value=9 cleared from provisional, "
             "value=7 entries remain",
             [(e.entity_id, e.attr_type, e.value_idx, e.episode_id)
              for e in pm.entries]
             == [("e1", "color", 7, 1), ("e1", "color", 7, 2)])

    # T16: audit state_before/state_after schema
    bank = _D7_Bank();  bank.empty_slot("e1", "color", version=2)
    si = _D7_StabilityIndex()
    pm = ProvisionalMemory()
    pm.entries = [_mk("e1", "color", 9, 2), _mk("e1", "color", 9, 3)]
    log = []
    _v15_7a_promote(pm, bank, si, current_episode=4, audit=log)
    rec = log[0]
    _assert7("T16 audit operation = PROMOTE", rec.operation == "PROMOTE")
    _assert7("T16 audit state_before.bank_present=False",
             rec.state_before["bank_present"] is False)
    _assert7("T16 audit state_before.bank_value_idx=-1",
             rec.state_before["bank_value_idx"] == -1)
    _assert7("T16 audit state_before.bank_version=2",
             rec.state_before["bank_version"] == 2)
    _assert7("T16 audit state_after.bank_present=True",
             rec.state_after["bank_present"] is True)
    _assert7("T16 audit state_after.bank_value_idx=9",
             rec.state_after["bank_value_idx"] == 9)
    _assert7("T16 audit state_after.bank_version=3 (bumped)",
             rec.state_after["bank_version"] == 3)
    _assert7("T16 audit state_after.stability_committed_episode=4",
             rec.state_after["stability_committed_episode"] == 4)
    _assert7("T16 audit state_after.provisional_entries_for_value=0",
             rec.state_after["provisional_entries_for_value"] == 0)
    _assert7("T16 audit episode_id = current_episode (4)",
             rec.episode_id == 4)

    # T17: GATE 3 ROLLUP — false_promote_rate = 0 across negative cases
    print()
    print("        [GATE 3 ROLLUP] false_promote_rate target = 0")
    print(f"        T4 insufficient confirmations: op must be 0")
    print(f"        T5 L4 anti-inflation:          op must be 0")
    print(f"        T6 age<K_age:                   op must be 0 (first call)")
    print(f"        T7 bank stable diff value:     op must be 0")
    print(f"        T8 entity not in bank:         op must be 0")
    g3_negative_failures = [m for m in failures7
                              if any(k in m for k in
                                     ["T4 GATE 3:", "T5 GATE 3 L4:",
                                      "T6 age<K_age", "T7 GATE 3:",
                                      "T8 entity not in bank"])]
    _assert7("T17 GATE 3: false_promote_rate = 0 across negatives",
             len(g3_negative_failures) == 0,
             f"violations: {g3_negative_failures}")

    print()
    if failures7:
        print(f"[v15.7a Pas 7a D.7] FAIL: {len(failures7)} self-check(s)"
              f" failed")
        for m in failures7:
            print(f"        - {m}")
        print()
        print("        GATE 3 STATUS: BLOCKED — D.8 wiring MUST NOT proceed")
    else:
        print("[v15.7a Pas 7a D.7] PASS: all promote self-checks green")
        print()
        print("        GATE 3 STATUS: PASS — false_promote_rate=0 confirmed")
        print("        D.8 wiring may proceed.")


# ===========================================================================
# v15.7a PAS 7A — Section D8: end_episode wiring (CommitArbiterPas7a)
# ===========================================================================
#
# Wires the consolidator pipeline into CommitArbiterPas6.end_episode by
# subclassing it (zero touch on Pas 6 critical path). After Pas 2/6 finalize
# completes, the consolidator runs synchronously in fixed order:
#
#     finalize (Pas 2/6) -> reconcile -> prune -> retrograde -> promote
#
# All in-episode behavior (write_fact, RoMR, dual conflict rule, cross-episode
# challenger detection) is inherited unchanged. Read path is untouched.
#
# Audit log accumulates ACROSS all episodes for the lifetime of the arbiter
# instance. Per-episode counts are stashed on `last_consolidator_ops` for the
# evaluator (D9) to assert against expected_ops.
#
# Order proof: pipeline call order matches README C3 -> tested in D8 self-check
# T1 by inspecting audit log operation sequence in a single end_episode.
# ===========================================================================


def _v15_7a_run_consolidator_pipeline(provisional_memory: "ProvisionalMemory",
                                          bank: "DeterministicObjectBank",
                                          stability_index: "BankStabilityIndex",
                                          current_episode: int,
                                          audit: List[V15_7a_ConsolidationRecord],
    ...
    """Run the four consolidator ops in the README C3 order.

    Returns op counts keyed by operation name.
    """
    n_reconcile = _v15_7a_reconcile(provisional_memory, current_episode,
                                         audit)
    n_prune = _v15_7a_prune(provisional_memory, current_episode, audit,
                                K_stale=K_stale)
    n_retrograde = _v15_7a_retrograde(provisional_memory, bank,
                                           stability_index,
                                           current_episode, audit, M=M)
    n_promote = _v15_7a_promote(provisional_memory, bank, stability_index,
                                     current_episode, audit, N=N,
                                     K_age=K_age)
    return {
        "RECONCILE":  n_reconcile,
        "PRUNE":      n_prune,
        "RETROGRADE": n_retrograde,
        "PROMOTE":    n_promote,
    }


# Guarded class definition: only build if the Pas 6 base class exists at
# import time (the scratch self-check runner does NOT import Pas 6, so we
# skip the class definition there to avoid a NameError on import).
if "CommitArbiterPas6" in globals():

    class CommitArbiterPas7a(CommitArbiterPas6):
        """Pas 7a arbiter: identical to Pas 6 in-episode; runs consolidator
        pipeline at end_episode after the Pas 2/6 finalize.

        Held INVARIANTS (all from Pas 6, none broken):
          - bank/provisional/stability touched ONLY at end_episode
          - episode buffer protocol unchanged
          - dual conflict rule applied unchanged
          - RoMR fact-side filtering unchanged
          - read path untouched
          - query path untouched

        New (Pas 7a only):
          - consolidation_audit_log: List[V15_7a_ConsolidationRecord]
            accumulates across all end_episode calls
          - last_consolidator_ops: Dict[str, int] from the most recent
            end_episode call (None before first end_episode)
        """

        def __init__(self, *args,
                         consolidation_audit_log: Optional[
                             List[V15_7a_ConsolidationRecord]] = None,
                         **kwargs):
            super().__init__(*args, **kwargs)
            self.consolidation_audit_log: List[V15_7a_ConsolidationRecord] = (
                consolidation_audit_log
                if consolidation_audit_log is not None
                else []
            )
            self.last_consolidator_ops: Optional[Dict[str, int]] = None

        def end_episode(self, entity_emb_fn, class_emb_fn, value_emb_fn):
            # Capture episode_id BEFORE super() runs (super calls
            # episode_buffer.end_episode which sets is_active=False but
            # preserves episode_id; the documented contract is that the
            # returned finalize.episode_id == the ended episode).
            episode_id_before = self.episode_buffer.episode_id

            # Run Pas 2/6 finalize first (mutates bank/provisional/stability).
            finalize = super().end_episode(entity_emb_fn, class_emb_fn,
                                                  value_emb_fn)

            # Episode id used for the consolidator pass: the just-ended one.
            ended_id = (finalize.episode_id
                              if finalize.episode_id is not None
                              else episode_id_before)

            # Run consolidator pipeline (reconcile -> prune -> retrograde
            # -> promote). Pipeline mutates bank/provisional/stability and
            # appends to consolidation_audit_log.
            ops = _v15_7a_run_consolidator_pipeline(
                self.provisional_memory,
                self.bank,
                self.stability_index,
                current_episode=ended_id,
                audit=self.consolidation_audit_log,
            )
            self.last_consolidator_ops = ops
            return finalize

    print("[v15.7a Pas 7a] Section D8: CommitArbiterPas7a wired "
          "(Pas 2/6 finalize -> reconcile -> prune -> retrograde -> promote)")
else:
    print("[v15.7a Pas 7a] Section D8: CommitArbiterPas7a NOT instantiated "
          "(Pas 6 base class not in scope; pipeline helper available)")


# ---- Dispatch (gated, defaults to skip) ------------------------------------
V15_7A_D8_MODE = os.environ.get("V15_7A_D8_MODE", "skip")
if V15_7A_D8_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.8] Running pipeline self-check"
          " (V15_7A_D8_MODE=run) — order + L1 simulation")
    print(SEP)
    ...
    @dataclass
    class _D8_ObjectRecord:
        entity_id:  str
        attr_slots: Dict[str, _D8_AttrSlot] = field(default_factory=dict)

    class _D8_Bank:
        def __init__(self):
            self._records: Dict[int, _D8_ObjectRecord] = {}
            self._entity_id_to_slot: Dict[str, int] = {}
            self._next_slot = 0
        ...
        def get_record(self, slot: int) -> Optional[_D8_ObjectRecord]:
            return self._records.get(slot)
        ...
        def commit_slot(self, ent: str, attr: str, value_idx: int,
            ...
            if slot is None:
                slot = self._next_slot
                self._next_slot += 1
                self._records[slot] = _D8_ObjectRecord(entity_id=ent)
                self._entity_id_to_slot[ent] = slot
            self._records[slot].attr_slots[attr] = _D8_AttrSlot(
                present=True, value_idx=value_idx, version=version,
                write_step=write_step,
            )

    class _D8_StabilityIndex:
        def __init__(self):
            self.committed_episode: Dict[Tuple[str, str], int] = {}
    ...
    def _assert8(name: str, cond: bool, detail: str = ""):
        if cond:
            print(f"        [OK]   {name}")
        else:
            msg = f"{name} FAILED {detail}".strip()
            failures8.append(msg)
            print(f"        [FAIL] {msg}")

    # T1: pipeline ORDER — single end_episode that exercises all 4 ops
    # produces audit records in fixed order
    bank = _D8_Bank()
    bank.commit_slot("e1", "color", 5, version=1)        # red committed
    bank.commit_slot("e2", "color", 5, version=1)        # for stale prune ent
    si = _D8_StabilityIndex()
    si.mark_committed("e1", "color", 1)
    si.mark_committed("e2", "color", 1)
    pm = ProvisionalMemory()
    # e1: blue challenger across eps 2 and 3 (will trigger reconcile + retro)
    pm.entries = [
        _mk("e1", "color", 9, 2),
        _mk("e1", "color", 9, 3),
        # e_stale: provisional value at ep1 only -> will trigger prune at ep4
        _mk("e_stale", "color", 7, 1),
    ]
    log: List[V15_7a_ConsolidationRecord] = []
    # Run pipeline at end_ep=4. Slot e1 is touched in ep3 not ep4, so
    # reconcile won't fire on it at ep=4. Use ep=3 for full chain instead.
    # Re-set up: at end_ep3, exercise reconcile (e1 slot touched ep3,
    # 2 entries -> fires); prune (e_stale last_act=1, diff=2, K=3 -> no);
    # retrograde (e1 committed=red, blue 2 eps -> fires); promote
    # (e1 blue eligible by N+age but blocked by intra-pas exclusion).
    ops = _v15_7a_run_consolidator_pipeline(
        pm, bank, si, current_episode=3, audit=log
    )
    op_seq = [r.operation for r in log]
    _assert8("T1 order: RECONCILE before RETROGRADE",
             "RECONCILE" in op_seq
             and op_seq.index("RECONCILE") < op_seq.index("RETROGRADE"))
    _assert8("T1 order: PRUNE not fired (no stale slot at ep3)",
             "PRUNE" not in op_seq)
    _assert8("T1 order: RETROGRADE fired (e1 demoted)",
             "RETROGRADE" in op_seq
             and any(r.operation == "RETROGRADE" and r.entity_id == "e1"
                     for r in log))
    _assert8("T1 order: PROMOTE blocked by intra-pas exclusion",
             not any(r.operation == "PROMOTE" for r in log))
    _assert8("T1 order: counts dict has all 4 keys",
             set(ops.keys()) == {"RECONCILE", "PRUNE", "RETROGRADE",
                                   "PROMOTE"})
    _assert8("T1 order: counts {RECONCILE=1, PRUNE=0, RETRO=1, PROMOTE=0}",
             ops == {"RECONCILE": 1, "PRUNE": 0,
                       "RETROGRADE": 1, "PROMOTE": 0},
             f"got {ops}")

    # T2: L1 SIMULATION — full 4-episode sequence, summed expected_ops
    # matches L1 expected_ops {RECONCILE: 1, RETROGRADE: 1, PROMOTE: 1}.
    # We simulate the cross-episode state transitions that Pas 2/6 finalize
    # would produce, then call the pipeline at each end_episode.
    bank = _D8_Bank()
    si = _D8_StabilityIndex()
    pm = ProvisionalMemory()
    log = []
    summed = {"RECONCILE": 0, "PRUNE": 0, "RETROGRADE": 0, "PROMOTE": 0}

    # ep1: write red -> commit_to_bank, mark stability. provisional empty.
    bank.commit_slot("e1", "color", 5, version=1, write_step=0)
    si.mark_committed("e1", "color", 1)
    ops1 = _v15_7a_run_consolidator_pipeline(pm, bank, si, 1, log)
    for k, v in ops1.items(): summed[k] += v

    # ep2: blue cross-episode challenger -> goes to provisional@ep2.
    pm.entries.append(_mk("e1", "color", 9, 2))
    ops2 = _v15_7a_run_consolidator_pipeline(pm, bank, si, 2, log)
    for k, v in ops2.items(): summed[k] += v

    # ep3: another blue challenger -> provisional@ep3. end_ep3 fires:
    # reconcile (slot touched ep3, 2 entries), retrograde (M=2 met),
    # promote BLOCKED (intra-pas exclusion).
    pm.entries.append(_mk("e1", "color", 9, 3))
    ops3 = _v15_7a_run_consolidator_pipeline(pm, bank, si, 3, log)
    for k, v in ops3.items(): summed[k] += v

    # ep4: distractor. No new provisional for e1. end_ep4 fires:
    # promote (blue N=2, age=2, no intra-pas retrograde this episode).
    ops4 = _v15_7a_run_consolidator_pipeline(pm, bank, si, 4, log)
    for k, v in ops4.items(): summed[k] += v

    _assert8("T2 L1 sim ep3 RECONCILE: 1", ops3["RECONCILE"] == 1,
             f"got {ops3}")
    _assert8("T2 L1 sim ep3 RETROGRADE: 1", ops3["RETROGRADE"] == 1)
    _assert8("T2 L1 sim ep3 PROMOTE: 0 (intra-pas blocked)",
             ops3["PROMOTE"] == 0)
    _assert8("T2 L1 sim ep4 PROMOTE: 1 (1-episode delay honored)",
             ops4["PROMOTE"] == 1, f"got {ops4}")
    _assert8("T2 L1 sim summed expected_ops match",
             summed["RECONCILE"] == 1
             and summed["RETROGRADE"] == 1
             and summed["PROMOTE"] == 1
             and summed["PRUNE"] == 0,
             f"got {summed}")
    _assert8("T2 L1 sim final bank: blue committed (value=9)",
             bank.get_record(0).attr_slots["color"].value_idx == 9
             and bank.get_record(0).attr_slots["color"].present is True)
    _assert8("T2 L1 sim final stability: marked at ep4",
             si.episode_of("e1", "color") == 4)
    _assert8("T2 L1 sim final provisional: empty",
             len(pm.entries) == 0)

    # T3: L2 SIMULATION — 3 episodes, ends before promote. Summed:
    # {RECONCILE: 1, RETROGRADE: 1}.
    bank = _D8_Bank()
    si = _D8_StabilityIndex()
    pm = ProvisionalMemory()
    log = []
    ...
    bank.commit_slot("e1", "color", 5, version=1)
    si.mark_committed("e1", "color", 1)
    _v15_7a_run_consolidator_pipeline(pm, bank, si, 1, log)
    pm.entries.append(_mk("e1", "color", 9, 2))
    _v15_7a_run_consolidator_pipeline(pm, bank, si, 2, log)
    pm.entries.append(_mk("e1", "color", 9, 3))
    ops3 = _v15_7a_run_consolidator_pipeline(pm, bank, si, 3, log)
    for k, v in ops3.items(): summed[k] += v
    _assert8("T3 L2 sim ep3: {RECONCILE=1, RETROGRADE=1, PROMOTE=0}",
             ops3["RECONCILE"] == 1
             and ops3["RETROGRADE"] == 1
             and ops3["PROMOTE"] == 0,
             f"got {ops3}")
    _assert8("T3 L2 sim final bank: e1 demoted (present=False)",
             bank.get_record(0).attr_slots["color"].present is False)
    _assert8("T3 L2 sim final stability: cleared",
             ("e1", "color") not in si.committed_episode)
    _assert8("T3 L2 sim final provisional: blue still present",
             any(e.value_idx == 9 for e in pm.entries))

    # T4: L3 SIMULATION — completion across slots, all consolidator no-ops.
    # Summed: {0,0,0,0}. Gate 7: false_retrograde = 0 on completions.
    bank = _D8_Bank()
    si = _D8_StabilityIndex()
    pm = ProvisionalMemory()
    log = []
    summed = {"RECONCILE": 0, "PRUNE": 0, "RETROGRADE": 0, "PROMOTE": 0}
    bank.commit_slot("e1", "color", 5)
    si.mark_committed("e1", "color", 1)
    o = _v15_7a_run_consolidator_pipeline(pm, bank, si, 1, log)
    for k, v in o.items(): summed[k] += v
    bank.commit_slot("e1", "size", 3)
    si.mark_committed("e1", "size", 2)
    o = _v15_7a_run_consolidator_pipeline(pm, bank, si, 2, log)
    for k, v in o.items(): summed[k] += v
    bank.commit_slot("e1", "location", 7)
    si.mark_committed("e1", "location", 3)
    o = _v15_7a_run_consolidator_pipeline(pm, bank, si, 3, log)
    for k, v in o.items(): summed[k] += v
    _assert8("T4 L3 sim: zero ops total", summed == {
        "RECONCILE": 0, "PRUNE": 0, "RETROGRADE": 0, "PROMOTE": 0,
    }, f"got {summed}")
    _assert8("T4 L3 sim: all 3 slots still committed",
             bank.get_record(0).attr_slots["color"].present
             and bank.get_record(0).attr_slots["size"].present
             and bank.get_record(0).attr_slots["location"].present)

    # T5: L4 SIMULATION — anti-inflation. ep1 has intra-episode conflict
    # red-red-blue -> dual rule (empty stable + conflict) puts ALL to
    # provisional. ep2/3/4 distractor. Expected_ops: {RECONCILE: 1}.
    # No promote (1 distinct ep < N=2). No retrograde (no commit).
    # No prune at ep4 if K=3? last_act=ep1, diff at end_ep4 = 3 == K -> prune.
    # Hmm — L4 expected_ops in code is {"RECONCILE": 1, "PRUNE": 2}? Need
    # to check. From the test in C5, prune fires per entry; L4 has 2 entries.
    bank = _D8_Bank()
    si = _D8_StabilityIndex()
    pm = ProvisionalMemory()
    log = []
    summed = {"RECONCILE": 0, "PRUNE": 0, "RETROGRADE": 0, "PROMOTE": 0}
    # ep1: write red, write red, write blue -> all to provisional@ep1
    pm.entries.extend([_mk("e1", "color", 5, 1, ws=0),
                          _mk("e1", "color", 5, 1, ws=1),
                          _mk("e1", "color", 9, 1, ws=2)])
    o = _v15_7a_run_consolidator_pipeline(pm, bank, si, 1, log)
    for k, v in o.items(): summed[k] += v
    # ep2/3/4 distractor (no new provisional for e1)
    o = _v15_7a_run_consolidator_pipeline(pm, bank, si, 2, log)
    for k, v in o.items(): summed[k] += v
    o = _v15_7a_run_consolidator_pipeline(pm, bank, si, 3, log)
    for k, v in o.items(): summed[k] += v
    o = _v15_7a_run_consolidator_pipeline(pm, bank, si, 4, log)
    for k, v in o.items(): summed[k] += v
    _assert8("T5 L4 sim: RECONCILE=1 (ep1 collapse)",
             summed["RECONCILE"] == 1, f"got {summed}")
    _assert8("T5 L4 sim: PROMOTE=0 (anti-inflation: 1 distinct ep < N=2)",
             summed["PROMOTE"] == 0)
    _assert8("T5 L4 sim: RETROGRADE=0 (no committed slot)",
             summed["RETROGRADE"] == 0)
    # Prune: at end_ep4, last_act for slot=ep1, diff=3>=K=3 -> 2 prunes
    # (post-reconcile we have 2 entries: red@ep1, blue@ep1)
    _assert8("T5 L4 sim: PRUNE=2 at end_ep4 (per-entry counting)",
             summed["PRUNE"] == 2, f"got {summed}")

    # T6: L5 SIMULATION — stale prune. ep1 conflict, ep2-4 distractor.
    # Expected: {RECONCILE: 1, PRUNE: 2} (same prune semantics as L4).
    bank = _D8_Bank()
    si = _D8_StabilityIndex()
    pm = ProvisionalMemory()
    log = []
    summed = {"RECONCILE": 0, "PRUNE": 0, "RETROGRADE": 0, "PROMOTE": 0}
    pm.entries.extend([_mk("e1", "color", 5, 1, ws=0),
                          _mk("e1", "color", 9, 1, ws=1)])
    for ep in [1, 2, 3, 4]:
        o = _v15_7a_run_consolidator_pipeline(pm, bank, si, ep, log)
        for k, v in o.items(): summed[k] += v
    _assert8("T6 L5 sim: RECONCILE=1, PRUNE=2, others=0",
             summed == {"RECONCILE": 1, "PRUNE": 2,
                          "RETROGRADE": 0, "PROMOTE": 0},
             f"got {summed}")
    _assert8("T6 L5 sim: provisional empty after prune",
             len(pm.entries) == 0)

    # T7: pipeline helper signature
    import inspect
    sig = inspect.signature(_v15_7a_run_consolidator_pipeline)
    needed_params = {"provisional_memory", "bank", "stability_index",
                       "current_episode", "audit"}
    _assert8("T7 pipeline signature has all required params",
             needed_params.issubset(set(sig.parameters)),
             f"missing: {needed_params - set(sig.parameters)}")

    # T8: pipeline returns dict with all 4 keys, ordered reconcile->promote
    # Note: dict preservation of insertion order is guaranteed since 3.7.
    bank = _D8_Bank();  si = _D8_StabilityIndex()
    pm = ProvisionalMemory();  log = []
    ops = _v15_7a_run_consolidator_pipeline(pm, bank, si, 1, log)
    _assert8("T8 returns dict with 4 keys",
             list(ops.keys()) == ["RECONCILE", "PRUNE",
                                     "RETROGRADE", "PROMOTE"],
             f"got {list(ops.keys())}")
    _assert8("T8 empty inputs -> all zeros",
             ops == {"RECONCILE": 0, "PRUNE": 0,
                       "RETROGRADE": 0, "PROMOTE": 0})
    _assert8("T8 empty inputs -> empty audit",
             len(log) == 0)

    # T9: GATE 0 ROLLUP — pipeline NEVER mutates anything if all four ops
    # are no-ops. Bank, provisional, stability identical before/after.
    bank = _D8_Bank()
    bank.commit_slot("e_quiet", "color", 5, version=3)
    si = _D8_StabilityIndex()
    si.mark_committed("e_quiet", "color", 100)
    pm = ProvisionalMemory()
    log = []
    rec_before = bank.get_record(0).attr_slots["color"]
    snap_before = (rec_before.present, rec_before.value_idx,
                       rec_before.version)
    stab_before = dict(si.committed_episode)
    _v15_7a_run_consolidator_pipeline(pm, bank, si, 200, log)
    rec_after = bank.get_record(0).attr_slots["color"]
    snap_after = (rec_after.present, rec_after.value_idx,
                      rec_after.version)
    _assert8("T9 GATE 0: bank slot byte-identical when no ops fire",
             snap_before == snap_after,
             f"before={snap_before} after={snap_after}")
    _assert8("T9 GATE 0: stability_index byte-identical",
             stab_before == si.committed_episode)
    _assert8("T9 GATE 0: audit empty when no ops fire", len(log) == 0)

    print()
    if failures8:
        print(f"[v15.7a Pas 7a D.8] FAIL: {len(failures8)} self-check(s)"
              f" failed")
        for m in failures8:
            print(f"        - {m}")
        print()
        print("        D.8 STATUS: BLOCKED — D.9 full eval MUST NOT proceed")
    else:
        print("[v15.7a Pas 7a D.8] PASS: all pipeline self-checks green")
        print()
        print("        D.8 STATUS: PASS — pipeline order + L1/L2/L3/L4/L5 "
              "expected_ops match")
        print("        D.9 full eval may proceed.")


# ===========================================================================
# v15.7a PAS 7A — Section D9: Full evaluation (Gates 0-9)
# ===========================================================================
#
# Single end-to-end evaluator that runs all 10 acceptance gates. Requires
# the full Pas 6 stack (base_model + v15_1_memory + bank + parser). Runs
# only in Colab; the standalone scratch self-check verifies STRUCTURAL
# correctness of the evaluator (signature, gate count, return schema).
#
# Phase A — Pas 6 invariants under Pas 7a arbiter (Gates 0-2):
#   - Gate 0: trusted regression byte-identical (consolidator must be no-op
#             on single-episode trusted probes since N/M/K thresholds are
#             never met within one episode)
#   - Gate 1: wrong_commit <= 0.02 per F1-F5 family
#   - Gate 2: F2 safe_resolution >= 0.95
#
# Phase B — Consolidator behavior on L1-L5 (Gates 3-9):
#   - Gate 3: false_promote_rate = 0 across all sequences
#   - Gate 4: false_retrograde_rate = 0 across all sequences
#   - Gate 5: L1 promote_rate >= 0.95 (final committed == challenger)
#   - Gate 6: L2 retrograde_rate >= 0.90 (bank slot demoted at end_ep3)
#   - Gate 7: L3 false_retrograde_rate = 0 (completion is no-op)
#   - Gate 8: L4 promote_count = 0 (anti-inflation)
#   - Gate 9: L5 prune_count >= 1 per trial (stale slot pruned)
#
# A "false" op is one that fires on a sequence whose expected_ops does not
# include that operation (counted per-family vs the family's expected_ops).
# ===========================================================================


V15_7A_D9_GATES = {
    # gate_id -> (description, threshold_meaning)
    "Gate 0": "trusted regression byte-identical (Pas 6 single-episode)",
    "Gate 1": "wrong_commit_rate <= 0.02 on all 5 holdout families (F1-F5)",
    "Gate 2": "F2 safe_resolution >= 0.95",
    "Gate 3": "false_promote_rate = 0 across all L sequences",
    "Gate 4": "false_retrograde_rate = 0 across all L sequences",
    "Gate 5": "L1 promote_rate >= 0.95 (challenger reaches bank by end_ep4)",
    "Gate 6": "L2 retrograde_rate >= 0.90 (committed demoted at end_ep3)",
    "Gate 7": "L3 false_retrograde_rate = 0 (completion is consolidator no-op)",
    "Gate 8": "L4 promote_count = 0 (anti-inflation: 1 distinct ep < N=2)",
    "Gate 9": "L5 prune_count >= 1 per trial (stale slot pruned at end_ep4)",
}


def _v15_7a_d9_per_sequence_metrics(audit_records: List[V15_7a_ConsolidationRecord],
                                         expected_ops: Dict[str, int],
                                         family_tag: str
                                         ) -> Dict[str, int]:
    """Compute per-sequence metrics from the audit log of one sequence.

    A "false_op" is an operation that fired but is not present in the
    family's expected_ops (counted by op type, not by slot).
    """
    actual_counts = {"RECONCILE": 0, "PRUNE": 0,
                       "RETROGRADE": 0, "PROMOTE": 0,
                       "PROMOTE_SKIPPED": 0}
    for r in audit_records:
        actual_counts[r.operation] = actual_counts.get(r.operation, 0) + 1

    # False ops = max(0, actual - expected) per type
    false_promote = max(0, actual_counts.get("PROMOTE", 0)
                            - expected_ops.get("PROMOTE", 0))
    false_retrograde = max(0, actual_counts.get("RETROGRADE", 0)
                                - expected_ops.get("RETROGRADE", 0))

    return {
        "actual_RECONCILE":  actual_counts["RECONCILE"],
        "actual_PRUNE":      actual_counts["PRUNE"],
        "actual_RETROGRADE": actual_counts["RETROGRADE"],
        "actual_PROMOTE":    actual_counts["PROMOTE"],
        "actual_PROMOTE_SKIPPED": actual_counts["PROMOTE_SKIPPED"],
        "false_promote":     false_promote,
        "false_retrograde":  false_retrograde,
    }


def _v15_7a_json_safe(obj):
    """Recursively convert tuple keys (entity_id, attr_type) into stable
    string keys 'entity_id::attr_type' so json.dump can serialize the D9
    report. Strict serializer-only fix; semantics of the evaluation dict
    are unchanged.
    """
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            ...
            out[k] = _v15_7a_json_safe(v)
        return out
    if isinstance(obj, list):
        return [_v15_7a_json_safe(x) for x in obj]
    if isinstance(obj, tuple):
        return [_v15_7a_json_safe(x) for x in obj]
    return obj


def v15_7a_run_full_eval_d9(base_model, v15_1_memory,
                                 n_per_l_family: int = 20,
                                 seed: int = 20261103) -> Dict:
    """D.9 full eval: Gates 0-9 end-to-end.

    Runs Phase A (Pas 6 invariants under Pas 7a arbiter) + Phase B
    (longitudinal consolidator behavior on L1-L5).

    Returns dict with per-gate verdicts, per-family metrics, and overall
    verdict. Output suitable for v15_7a_d9_full_eval.json artifact.
    """
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.9] FULL EVALUATION — Gates 0..9")
    print(f"  n_per_l_family = {n_per_l_family}, seed = {seed}")
    print(SEP)

    # ===================================================================
    # Phase A: Pas 6 invariants under Pas 7a arbiter
    # ===================================================================
    print()
    print("[v15.7a Pas 7a D.9] Phase A: Pas 6 invariants (Gates 0-2)")
    print(SEP)
    ...
    # Trusted snapshot BEFORE — uses Pas 6's existing trusted probes
    bank = DeterministicObjectBank(capacity=64,
                                       d_model=base_model.config.hidden_dim)
    print("\n  Trusted snapshot BEFORE (Gate 0):")
    snap_before = _v15_5_snapshot_trusted(bank, base_model, v15_1_memory)
    for k, v in snap_before.items():
        print(f"    {k}: {v:.4f}")

    # Build Pas 7a arbiter for the holdout family runs
    bank.reset()
    provisional_memory = ProvisionalMemory()
    episode_buffer     = EpisodeBuffer()
    stability_index    = BankStabilityIndex()
    composer_traces: List[ComposerTrace] = []
    romr_traces:    List[RoMRResult]     = []
    consolidator_audit: List[V15_7a_ConsolidationRecord] = []
    arbiter = CommitArbiterPas7a(
        bank, provisional_memory, episode_buffer, stability_index,
        composer_trace_log=composer_traces,
        romr_trace_log=romr_traces,
        consolidation_audit_log=consolidator_audit,
    )
    reader = ReadArbiter(bank, provisional_memory)

    # Run F1..F5 single-episode families
    family_results: Dict[str, Dict] = {}
    seed_offset = 100000
    episode_counter = 8_000_000

    print(f"\n  Running 5 holdout families (n="
          f"{V15_5_HOLDOUT_CONFIG['n_per_family']} each)")
    for fname, gen in EXTERNAL_HOLDOUT_FAMILIES.items():
        print(f"    -> {fname}")
        rng = _rng_module.Random(V15_5_HOLDOUT_CONFIG["seed"] + seed_offset)
        outcomes = []
        for _ in range(V15_5_HOLDOUT_CONFIG["n_per_family"]):
            ep = gen(rng, ENC, class_map)
            bank.reset()
            provisional_memory.reset()
            episode_buffer.clear()
    ...
    # Trusted snapshot AFTER — must match BEFORE byte-identical (Gate 0)
    print("\n  Trusted snapshot AFTER (Gate 0):")
    bank.reset()
    snap_after = _v15_5_snapshot_trusted(bank, base_model, v15_1_memory)
    gate0_match, bad_k, vb, va = _v15_5_trusted_signatures_match(snap_before,
                                                                       snap_after)
    if gate0_match:
        print("    PASS: trusted regression byte-identical")
    else:
        print(f"    FAIL: regression on '{bad_k}': before={vb} after={va}")
    ...
    # ===================================================================
    print()
    print("[v15.7a Pas 7a D.9] Phase B: Consolidator on L1-L5 (Gates 3-9)")
    print(SEP)

    per_l_family: Dict[str, List[Dict]] = {}
    l_family_episode_counter = 9_000_000

    for family_name, gen in LONGITUDINAL_FAMILIES.items():
        print(f"\n  Running {family_name} (n={n_per_l_family})")
        rng = _rng_module.Random(seed + hash(family_name) % 10000)
        trials: List[Dict] = []

        for trial_idx in range(n_per_l_family):
            # Reset all state between sequences
            bank.reset()
            provisional_memory.reset()
            episode_buffer.clear()
            stability_index.reset()
            audit_marker = len(consolidator_audit)

            seq = gen(rng, ENC, class_map)
            per_episode_ops: List[Dict[str, int]] = []

            for ep in seq.episodes:
                arbiter.begin_episode(l_family_episode_counter)
                for j, fact_text in enumerate(ep.facts):
                    arbiter.write_fact(fact_text, ent_fn, cls_fn, val_fn,
                                          write_step=j)
                arbiter.end_episode(ent_fn, cls_fn, val_fn)
                per_episode_ops.append(dict(arbiter.last_consolidator_ops or {}))
                l_family_episode_counter += 1

            # Slice this trial's audit records
            trial_audit = consolidator_audit[audit_marker:]
            metrics = _v15_7a_d9_per_sequence_metrics(
                trial_audit, seq.expected_ops, seq.family_tag
            )
            # Snapshot final state
            observed = _v15_7a_snapshot_sequence_state(
                bank, provisional_memory,
                consolidator_ops_count={
                    k: sum(o.get(k, 0) for o in per_episode_ops)
                    for k in ("RECONCILE", "PRUNE",
                                "RETROGRADE", "PROMOTE")
                },
            )
            trials.append({
                "sequence_id":   seq.sequence_id,
                "family_tag":    seq.family_tag,
                "primary_entity_id": seq.primary_entity_id,
                "expected_ops":  dict(seq.expected_ops),
                "metrics":       metrics,
                "final_committed": dict(observed.bank_snapshot),
                "final_provisional_count": len(observed.provisional_entries),
                "expected_committed": dict(seq.expected_final_committed),
            })

        per_l_family[family_name] = trials

        # Per-family summary
        n_trials_total = len(trials)
        n_committed_match = sum(
            1 for t in trials
            if t["final_committed"] == t["expected_committed"]
        )
        n_promote_total = sum(t["metrics"]["actual_PROMOTE"] for t in trials)
        n_retrograde_total = sum(t["metrics"]["actual_RETROGRADE"]
                                       for t in trials)
        n_prune_total = sum(t["metrics"]["actual_PRUNE"] for t in trials)
        n_false_promote = sum(t["metrics"]["false_promote"] for t in trials)
        n_false_retrograde = sum(t["metrics"]["false_retrograde"]
                                       for t in trials)
        print(f"    committed_match: {n_committed_match}/{n_trials_total}  "
              f"PROMOTE={n_promote_total}  "
              f"RETROGRADE={n_retrograde_total}  "
              f"PRUNE={n_prune_total}  "
              f"false_promote={n_false_promote}  "
              f"false_retrograde={n_false_retrograde}")
    ...
    print()
    print(SEP)
    print("=== V15.7A PAS 7A D.9 ACCEPTANCE GATES (10) ===")
    print(SEP)

    verdicts: Dict[str, bool] = {}

    # Gate 0
    verdicts["Gate 0"] = bool(gate0_match)

    # Gate 1: wrong_commit on every family
    g1_pass = all(r["wrong_commit_rate"] <= 0.02
                      for r in family_results.values())
    verdicts["Gate 1"] = g1_pass

    # Gate 2: F2 safe_resolution
    f2 = family_results.get("F2_multiword_entities", {})
    f2_safe = (f2.get("commit_correct_rate", 0)
                  + f2.get("provisional_correct_rate", 0))
    verdicts["Gate 2"] = (f2_safe >= 0.95)

    # Gate 3: false_promote_rate = 0 across all L sequences
    total_false_promote = sum(
        t["metrics"]["false_promote"]
        for trials in per_l_family.values() for t in trials
    )
    verdicts["Gate 3"] = (total_false_promote == 0)

    # Gate 4: false_retrograde_rate = 0 across all L sequences
    total_false_retrograde = sum(
        t["metrics"]["false_retrograde"]
        for trials in per_l_family.values() for t in trials
    )
    verdicts["Gate 4"] = (total_false_retrograde == 0)

    # Gate 5: L1 promote_rate >= 0.95
    l1_trials = per_l_family.get("L1_promote_cycle", [])
    l1_promote_rate = (
        sum(1 for t in l1_trials if t["metrics"]["actual_PROMOTE"] >= 1)
        / max(1, len(l1_trials))
    )
    verdicts["Gate 5"] = (l1_promote_rate >= 0.95)

    # Gate 6: L2 retrograde_rate >= 0.90
    l2_trials = per_l_family.get("L2_retrograde_only", [])
    l2_retro_rate = (
        sum(1 for t in l2_trials if t["metrics"]["actual_RETROGRADE"] >= 1)
        / max(1, len(l2_trials))
    )
    verdicts["Gate 6"] = (l2_retro_rate >= 0.90)

    # Gate 7: L3 false_retrograde_rate = 0
    l3_trials = per_l_family.get("L3_completion", [])
    l3_false_retro = sum(t["metrics"]["actual_RETROGRADE"]
                              for t in l3_trials)
    verdicts["Gate 7"] = (l3_false_retro == 0)

    # Gate 8: L4 promote_count = 0
    l4_trials = per_l_family.get("L4_no_inflation", [])
    l4_promote = sum(t["metrics"]["actual_PROMOTE"] for t in l4_trials)
    verdicts["Gate 8"] = (l4_promote == 0)

    # Gate 9: L5 prune_count >= 1 per trial
    l5_trials = per_l_family.get("L5_stale_prune", [])
    l5_prune_per_trial = [t["metrics"]["actual_PRUNE"] for t in l5_trials]
    verdicts["Gate 9"] = all(p >= 1 for p in l5_prune_per_trial) \
                              if l5_prune_per_trial else False

    for gid, ok in verdicts.items():
        mark = "✓" if ok else "✗"
        print(f"  {mark} {gid}: {V15_7A_D9_GATES[gid]}")

    overall = all(verdicts.values())
    print()
    print(SEP)
    if overall:
        print("FINAL D.9 VERDICT: PAS 7A SEALED (all 10 gates green)")
    else:
        failed = [g for g, ok in verdicts.items() if not ok]
        print(f"FINAL D.9 VERDICT: BLOCKED — failed gates: {failed}")
    print(SEP)

    return {
        "config":         {"n_per_l_family": n_per_l_family, "seed": seed},
        "verdicts":       verdicts,
        "overall":        overall,
        "phase_a": {
            "trusted_snapshot_before": dict(snap_before),
            "trusted_snapshot_after":  dict(snap_after),
            "trusted_match":           bool(gate0_match),
            "family_results":          family_results,
        },
        "phase_b": {
            "per_l_family":             per_l_family,
            "total_false_promote":      total_false_promote,
            "total_false_retrograde":   total_false_retrograde,
            "l1_promote_rate":          l1_promote_rate,
            "l2_retrograde_rate":       l2_retro_rate,
            "l3_false_retrograde_count": l3_false_retro,
            "l4_promote_count":         l4_promote,
            "l5_prune_per_trial":       l5_prune_per_trial,
        },
    }


print("[v15.7a Pas 7a] Section D9: Full evaluator defined "
      "(Gates 0-9; activate via V15_7A_D9_MODE=run)")


# ---- Dispatch (gated, defaults to skip) ------------------------------------
V15_7A_D9_MODE = os.environ.get("V15_7A_D9_MODE", "skip")
if V15_7A_D9_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.9] Running FULL EVAL (V15_7A_D9_MODE=run)")
    print(SEP)
    if "base_model_v15_6" not in globals() or "v15_1_memory_v15_6" not in globals():
        print("[v15.7a Pas 7a D.9] base_model_v15_6 / v15_1_memory_v15_6 not in scope")
        print("[v15.7a Pas 7a D.9] D.9 requires the full Pas 6 stack — skipping.")
    else:
        d9_result = v15_7a_run_full_eval_d9(
            base_model_v15_6, v15_1_memory_v15_6, n_per_l_family=20,
        )
        V15_7A_DIR         = os.path.join(PROJECT_ROOT, "v15_7a")
        V15_7A_RESULTS_DIR = os.path.join(V15_7A_DIR, "results")
        os.makedirs(V15_7A_RESULTS_DIR, exist_ok=True)
        ...
        with open(d9_path, "w") as f:
            json.dump(_v15_7a_json_safe(d9_result), f, indent=2, default=str)
        print(f"\n[v15.7a Pas 7a D.9] saved: {d9_path}")


# ---- D9 STRUCTURAL self-check (gated, defaults to skip) -------------------
# Validates locally what does NOT need the Pas 6 model: gate inventory,
# per-sequence metrics math, and evaluator signature. Full integration must
# be run in Colab via V15_7A_D9_MODE=run.
V15_7A_D9_STRUCT_MODE = os.environ.get("V15_7A_D9_STRUCT_MODE", "skip")
if V15_7A_D9_STRUCT_MODE == "run":
    print()
    print(SEP)
    print("[v15.7a Pas 7a D.9-struct] Running structural self-check"
          " (V15_7A_D9_STRUCT_MODE=run)")
    print(SEP)

    failures9: List[str] = []

    def _assert9(name: str, cond: bool, detail: str = ""):
        if cond:
            print(f"        [OK]   {name}")
        else:
            msg = f"{name} FAILED {detail}".strip()
            failures9.append(msg)
            print(f"        [FAIL] {msg}")

    # T1: gate inventory has exactly 10 gates, ids "Gate 0".."Gate 9"
    expected_ids = {f"Gate {i}" for i in range(10)}
    _assert9("T1 V15_7A_D9_GATES has 10 entries",
             len(V15_7A_D9_GATES) == 10)
    _assert9("T1 gate ids = {Gate 0..Gate 9}",
             set(V15_7A_D9_GATES.keys()) == expected_ids,
             f"got {set(V15_7A_D9_GATES.keys())}")

    # T2: per-sequence metrics — happy L1 trial
    # Audit records simulating L1: 1 RECONCILE @ ep3, 1 RETROGRADE @ ep3,
    # 1 PROMOTE @ ep4. Expected_ops = {RECONCILE:1, RETROGRADE:1, PROMOTE:1}.
    # No false ops.
    audit = [
        V15_7a_ConsolidationRecord(
            episode_id=3, operation="RECONCILE",
            entity_id="e1", attr_type="color", value_idx=None,
        ...
        V15_7a_ConsolidationRecord(
            episode_id=4, operation="PROMOTE",
            entity_id="e1", attr_type="color", value_idx=9,
            reason="", state_before={}, state_after={},
        ),
    ]
    expected = {"RECONCILE": 1, "RETROGRADE": 1, "PROMOTE": 1}
    m = _v15_7a_d9_per_sequence_metrics(audit, expected, "L1_promote_cycle")
    _assert9("T2 L1: actual_RECONCILE=1", m["actual_RECONCILE"] == 1)
    _assert9("T2 L1: actual_RETROGRADE=1", m["actual_RETROGRADE"] == 1)
    _assert9("T2 L1: actual_PROMOTE=1", m["actual_PROMOTE"] == 1)
    _assert9("T2 L1: false_promote=0", m["false_promote"] == 0)
    _assert9("T2 L1: false_retrograde=0", m["false_retrograde"] == 0)

    # T3: per-sequence metrics — false_promote
    # L4 expected_ops has PROMOTE=0 (anti-inflation). If actual fires,
    # false_promote = actual.
    audit_bad_l4 = [
        V15_7a_ConsolidationRecord(
            episode_id=4, operation="PROMOTE",
            entity_id="e1", attr_type="color", value_idx=5,
    ...
    m = _v15_7a_d9_per_sequence_metrics(
        audit_bad_l4,
        {"RECONCILE": 1, "PRUNE": 2}, "L4_no_inflation"
    )
    _assert9("T3 L4 violation: false_promote=1",
             m["false_promote"] == 1, f"got {m['false_promote']}")

    # T4: per-sequence metrics — false_retrograde
    audit_bad_l3 = [
        V15_7a_ConsolidationRecord(
            episode_id=2, operation="RETROGRADE",
            entity_id="e1", attr_type="color", value_idx=5,
            reason="", state_before={}, state_after={},
        ),
    ]
    m = _v15_7a_d9_per_sequence_metrics(
        audit_bad_l3, {}, "L3_completion"
    )
    _assert9("T4 L3 violation: false_retrograde=1",
             m["false_retrograde"] == 1)

    # T5: per-sequence metrics — PROMOTE_SKIPPED counted separately
    audit_skips = [
        V15_7a_ConsolidationRecord(
            episode_id=4, operation="PROMOTE_SKIPPED",
            entity_id="e1", attr_type="color", value_idx=9,
            reason="bank stable diff value",
            state_before={}, state_after={},
        ),
    ]
    m = _v15_7a_d9_per_sequence_metrics(audit_skips, {}, "test")
    _assert9("T5 PROMOTE_SKIPPED counted but does NOT count as false_promote",
             m["actual_PROMOTE_SKIPPED"] == 1
             and m["actual_PROMOTE"] == 0
             and m["false_promote"] == 0)

    # T6: evaluator function signature
    import inspect
    sig = inspect.signature(v15_7a_run_full_eval_d9)
    _assert9("T6 evaluator signature has base_model parameter",
             "base_model" in sig.parameters)
    _assert9("T6 evaluator signature has v15_1_memory parameter",
             "v15_1_memory" in sig.parameters)
    _assert9("T6 evaluator signature has n_per_l_family parameter",
             "n_per_l_family" in sig.parameters)

    # T7: pipeline helper signature unchanged after D9 added
    sig2 = inspect.signature(_v15_7a_run_consolidator_pipeline)
    _assert9("T7 pipeline helper still has 5+ params",
             len(sig2.parameters) >= 5)

    # T8: V15_7A_D9_GATES descriptions are non-empty strings
    _assert9("T8 all gate descriptions are non-empty",
             all(isinstance(v, str) and len(v) > 0
                 for v in V15_7A_D9_GATES.values()))

    print()
    if failures9:
        print(f"[v15.7a Pas 7a D.9-struct] FAIL: {len(failures9)} self-check(s)"
              f" failed")
        for m in failures9:
            print(f"        - {m}")
    else:
        print("[v15.7a Pas 7a D.9-struct] PASS: structural self-checks green")
        print("        Full Gate 0-9 evaluation requires Colab + Pas 6 stack;")
        print("        activate with V15_7A_D9_MODE=run on the full pipeline.")
