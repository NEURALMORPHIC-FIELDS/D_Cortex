# -*- coding: utf-8 -*-
# ===========================================================================
# D_Cortex v2.0-alpha -- Step 2: Training + Memory Validation (v2)
...
# ===========================================================================
#
# CHANGES vs v1 (review fixes applied):
#   [FIX-1 CRITICAL] torch.cuda.get_device_properties(0).total_memory
#                    (was .total_mem which does not exist -> AttributeError
#                     before training even starts)
#   [FIX-2] Double-checkpoint guard: if step+1 was just saved in eval block,
#           skip the ckpt_every save in the same step.
#   [FIX-3] Gate entropy loss: margin-based penalty max(0, H_min - H)
#           with H_min = 1.0. Entropy is penalized ONLY when it drops
#           below the margin, never pushed further down. Replaces the
#           naive direct-entropy-minimization that risked premature
#           collapse toward one-hot routing.
#   [FIX-4] Curriculum ramp: starts at step 500 with 5%, rises to 20%
#           by step 1500, reaches 50% by step 3500. Gives the memory
#           subsystem a longer training horizon with curriculum signal.
#   [FIX-5] Conflict test uses the CORRECT diff-vector mechanism
#           (v2 written with is_conflict=True on a bank already holding v1
#            with similar keys -> stored value is v2 - v1, verified by
#            cosine similarity vs expected).
#   [FIX-6] EpisodeSSM included in SUBMODULE_GROUPS for gradient tracking.
#   [FIX-7] _gate_accum cleared at start of ablation (cosmetic).
#
...
class TrainConfig:
    ...
    # [FIX-3] Margin entropy config: penalize only when gate entropy
    # drops below gate_entropy_min_H. Never pushes entropy lower.
    gate_entropy_w: float = 0.02     # weight on the margin violation
    gate_entropy_min_H: float = 1.0  # ~56% of max entropy ln(6)=1.79
    ...
    # [FIX-4] Curriculum ramp replaces curriculum_start + fixed ratio.
    # ramp = [(step, ratio), ...] linearly interpolated between anchors.
    curriculum_ramp: Tuple[Tuple[int, float], ...] = (
        (0,    0.00),
        (500,  0.05),
        (1500, 0.20),
        (3500, 0.50),
...
def curriculum_ratio_at(step: int) -> float:
    """[FIX-4] Linear interpolation over curriculum_ramp anchors."""
    anchors = TC.curriculum_ramp
    if step <= anchors[0][0]:
        return anchors[0][1]
    if step >= anchors[-1][0]:
        return anchors[-1][1]
...
print(f"[INFO] Curriculum ramp: {TC.curriculum_ramp}")
...
def compute_loss(
    ...
    """LM cross-entropy + MARGIN gate entropy auxiliary loss.

    [FIX-3] Instead of directly minimizing entropy (which pushes routing
    toward one-hot and risks premature collapse), we penalize ONLY when
    entropy drops below gate_entropy_min_H. The penalty is
    max(0, H_min - H). When H >= H_min, loss contribution is zero.

    Returns (total_loss_tensor, lm_loss_scalar, gate_entropy_scalar,
             margin_penalty_scalar).
    """
    B, T, V = logits.shape
    ...
    if gp is not None:
        ...
        gate_ent_val = H.item()
        # Margin penalty: only active when H < H_min
        margin_pen = F.relu(TC.gate_entropy_min_H - H)
        margin_val = margin_pen.item()

    total = lm_loss + TC.gate_entropy_w * margin_pen
    return total, lm_loss.item(), gate_ent_val, margin_val
...
M_curric: List[float] = []   # curriculum ratio per step
E_steps: List[int] = []
E_loss: List[float] = []
E_ppl: List[float] = []
loss_log: List[float] = []
...
for step in range(start_step, TC.total_steps):
    ...
    # [FIX-4] Curriculum ratio from ramp
    curric_ratio = curriculum_ratio_at(step)
    ...
    for _mi in range(TC.grad_accum):
        if curric_ratio > 0 and random.random() < curric_ratio:
        ...
        with torch.amp.autocast('cuda', dtype=DTYPE):
            ...
            loss, lm_val, ge_val, margin_val = compute_loss(logits, y)
    ...
    if step % TC.log_every == 0 or step == start_step:
        ...
        print(
            ...
            f"ge={acc_ge:.3f} (pen={acc_margin:.3f}) | "
            f"curric={curric_ratio:.2f} | gn={grad_norm:.2f} | "
...
fig.suptitle(
    'D_Cortex v2.0-alpha -- Step 2 (v2): Training + Memory Validation',
    fontsize=14, y=0.98,
)
...
for s, r in TC.curriculum_ramp:
    if 0 < s < TC.total_steps:
        ax.axvline(x=s, color='red', ls=':', alpha=0.4)
ax.set_xlabel('Step')
ax.set_ylabel('LM Loss')
ax.set_title('Training Loss (dashed red = curriculum ramp anchors)')
...
# 3. Gate entropy + margin
ax = axes[0, 2]
ax.plot(M_steps, M_ge, alpha=0.6, color='green', label='H (entropy)')
ax.plot(M_steps, M_margin, alpha=0.6, color='red',
        label=f'max(0, {TC.gate_entropy_min_H:.2f} - H)')
max_ent = math.log(6)
ax.axhline(y=max_ent, color='gray', ls='--', alpha=0.5, label=f'ln(6)={max_ent:.2f}')
ax.axhline(y=TC.gate_entropy_min_H, color='orange', ls=':', alpha=0.7,
           label=f'H_min={TC.gate_entropy_min_H:.2f}')
ax.set_xlabel('Step')
ax.set_ylabel('Nats')
ax.set_title('Writer Gate Entropy (monitoring) + Margin Penalty (loss)')
ax.grid(True, alpha=0.3)
...
# 7. Learning rate + curriculum
ax = axes[2, 0]
ax.plot(M_steps, M_lr, color='teal', label='LR')
ax2 = ax.twinx()
ax2.plot(M_steps, M_curric, color='purple', alpha=0.6, label='curriculum ratio')
ax2.set_ylabel('Curriculum ratio', color='purple')
ax.set_xlabel('Step')
ax.set_ylabel('LR', color='teal')
ax.set_title('LR Schedule + Curriculum Ramp')
ax.grid(True, alpha=0.3)
...
plt.tight_layout()
plot_path = os.path.join(RESULTS_DIR, 'step2v2_training_report.png')
...
report = {
    ...
    'step': 'Step 2 v2: Training + Memory Validation (review fixes applied)',
    'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
    'fixes_applied': [
        'FIX-1: total_memory (was total_mem)',
        'FIX-2: double-checkpoint guard',
        'FIX-3: margin entropy penalty (H_min=1.0) replaces direct minimization',
        'FIX-4: curriculum ramp (500->1500->3500) replaces fixed start',
        'FIX-5: conflict test verifies diff-vector cosine vs expected',
        'FIX-6: episode_ssm added to submodule gradient tracking',
        'FIX-7: _gate_accum cleared at start of ablation',
    ],
    'hardware': {
        'gpu': GPU_NAME,
    ...
    'data': {
        ...
        'curriculum_ramp': TC.curriculum_ramp,
    },
    'training': {
    ...
    'gate_entropy': {
        ...
        'H_max_uniform': round(math.log(6), 4),
        'H_initial': round(M_ge[0], 4) if M_ge else None,
        'H_final': round(M_ge[-1], 4) if M_ge else None,
        'margin_penalty_final': round(M_margin[-1], 4) if M_margin else None,
    },
    'memory_ablation': {
        'method': 'same-batch short-term priming (seed=12345)',
        'interpretation': 'local priming effect, not durable memory utility',
...
report_path = os.path.join(RESULTS_DIR, 'step2v2_training_report.json')
with open(report_path, 'w', encoding='utf-8') as f:
    json.dump(report, f, indent=2, default=str)

print(SEP)
print("D_CORTEX v2.0-alpha -- STEP 2 v2 REPORT")
print(SEP)
print(f"  Fixes applied   : 7 (see report JSON)")
...
if M_ge:
    print(f"  Gate entropy    : {M_ge[0]:.4f} -> {M_ge[-1]:.4f} "
          f"(H_min={TC.gate_entropy_min_H}, max ln(6)={max_ent:.2f})")
print(f"  Ablation delta  : {abl_delta:+.4f} (short-term priming, not durable)")
print(f"  Conflict test   : mechanism_functional="
      f"{conflict_results.get('mechanism_functional', 'N/A')}")
if conflict_results.get('diff_vector_cosine_vs_expected') is not None:
    print(f"                    diff_vector_cosine="
          f"{conflict_results['diff_vector_cosine_vs_expected']:.4f} "
          f"(expected >0.99)")
if M_subgrad:
...
print(SEP)
print("WHAT v2 PROVES (IF RUN SUCCEEDS):")
print("  - Training loop stable with margin-entropy instead of direct entropy min")
print("  - Gradients reach embeddings, backbone, memory, fusion, AND episode_ssm")
print("  - Writer gate entropy stays above H_min unless margin triggered")
print("  - Memory banks populate under ramped curriculum (from step 500)")
print("  - ConflictMemory diff-vector storage is numerically correct")
print("  - Same-batch ablation measures short-term priming (acknowledged)")
print()
print("WHAT IT STILL DOES NOT PROVE:")
print("  - Semantic routing correctness (requires Step 3 retrieval benchmarks)")
print("  - Durable long-horizon memory utility (requires structured recall tasks)")
print("  - That the MODEL generates conflict-inducing keys (direct probe only)")
print(SEP)
print("STATUS: READY TO RUN (fixes applied; still requires Colab A100 runtime)")
print(SEP)
