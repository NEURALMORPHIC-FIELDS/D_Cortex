# -*- coding: utf-8 -*-
# ===========================================================================
# D_Cortex v2.0-alpha -- Step 2: Training + Memory Validation (v3)
...
#
# CHANGES vs v2:
#   [FIX-8 CRITICAL] Writer was DEAD (zero gradient after margin entropy
#                    stabilized above H_min). Root cause: all values and
#                    keys are .detach() before bank storage, and gate
#                    entropy was the ONLY gradient path to the writer.
#                    Once H > H_min, margin penalty = 0, writer gradient = 0.
#
#   NEW: Two auxiliary losses that create direct gradient paths:
#
#   KEY-QUERY ALIGNMENT LOSS (w_key_align):
#     Forces writer keys to be FINDABLE by the query engine.
#     cos_sim(writer_keys, query_engine_keys) on same h_pool.
#     Gradient flows into: writer.key_ent/rel/typ + query_engine.proj_ent/rel/typ
#
#   VALUE COHERENCE LOSS (w_val_coherence):
#     Forces writer value_head to PRESERVE information from h_pool.
#     cos_sim(value_head(h), h_pool.detach())
#     Gradient flows into: writer.value_head (MLP), writer.norm
#
#   HOOK REMOVED: No longer need forward_hook on writer. model._aux dict
#                 provides all needed tensors with gradients intact.
#
# All v2 fixes (FIX 1-7) remain in effect.
# ===========================================================================
...
def compute_loss(
    ...
    [FIX-8] Three auxiliary components:
      1. Margin gate entropy: max(0, H_min - H)     -> prevents collapse
      2. Key-query alignment: 1 - cos(w_keys, q_keys) -> writer keys findable
      3. Value coherence: 1 - cos(value, h_pool)      -> values encode content

    Returns: (total_loss, lm_val, ge_val, margin_val, key_align_val, val_coh_val)
    """
    B, T, V = logits.shape
    ...
    return total, lm_loss.item(), ge_val, margin_val, key_align_val, val_coh_val
...
for step in range(start_step, TC.total_steps):
    ...
    for _mi in range(TC.grad_accum):
        ...
        with torch.amp.autocast('cuda', dtype=DTYPE):
            ...
            aux = model._aux
            loss, lm_val, ge_val, margin_val, ka_val, vc_val = compute_loss(
                logits, y, aux,
            )
        ...
        # Track gate distribution from model._aux (replaces hook)
        gp = aux.get('gate_probs')
        if gp is not None:
    ...
    if step % TC.log_every == 0 or step == start_step:
        ...
        print(
            ...
            f"ka={acc_keyalign:.3f} vc={acc_valcoh:.3f} | "
            f"cur={curric_ratio:.2f} | gn={grad_norm:.2f} | "
...
fig.suptitle(
    'D_Cortex v2.0-alpha -- Step 2 (v3): Training + Memory Aux Losses',
    fontsize=14, y=0.98,
)
...
plt.tight_layout()
plot_path = os.path.join(RESULTS_DIR, 'step2v3_training_report.png')
...
report = {
    ...
    'step': 'Step 2 v3: Training with writer gradient fix (key-query alignment + value coherence)',
    'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
    'fixes_applied': [
        'FIX-1: total_memory (was total_mem)',
        'FIX-2: double-checkpoint guard',
        'FIX-3: margin entropy penalty (H_min=1.0)',
        'FIX-4: curriculum ramp (500->1500->3500)',
...
report_path = os.path.join(RESULTS_DIR, 'step2v3_training_report.json')
with open(report_path, 'w', encoding='utf-8') as f:
    json.dump(report, f, indent=2, default=str)

print(SEP)
print("D_CORTEX v2.0-alpha -- STEP 2 v3 REPORT")
print(SEP)
print(f"  Fixes applied   : 8 (FIX-1..7 from v2 + FIX-8 writer aux losses)")
...
if M_valcoh:
    print(f"  Value coherence : {M_valcoh[0]:.4f} -> {M_valcoh[-1]:.4f} "
          f"(w={TC.w_val_coherence})")
print(f"  Ablation delta  : {abl_delta:+.4f} (short-term priming)")
...
print(SEP)
print("WHAT v3 CHANGES vs v2:")
print("  v2 problem: writer had ZERO gradient once margin entropy stabilized.")
print("              Gate stayed uniform, memory content was random, fusion")
print("              blocks ignored memory, Step 3 benchmarks showed delta=0%.")
print()
print("  v3 fix: two auxiliary losses create gradient paths through writer:")
print("    1. Key-query alignment: cos(writer_keys, query_keys)")
print("       -> writer keys become FINDABLE by the reader")
print("    2. Value coherence: cos(value_head(h), h_pool)")
print("       -> writer value_head preserves semantic content")
print()
print("WHAT TO LOOK FOR IN THESE RESULTS:")
print("  1. Writer submodule grad norm > 0 at ALL steps (was 0 in v2)")
print("  2. key_align loss should DECREASE over training")
print("  3. val_coherence loss should DECREASE over training")
print("  4. Gate distribution should become NON-UNIFORM (specialization)")
print("  5. Fusion block mem_gate should move AWAY from 0.5")
print("  6. Same-batch ablation delta should be > 0")
print("  7. Step 3 benchmarks (run separately) should show delta > 0")
print(SEP)
print("STATUS: READY TO RUN (requires fresh training, not resumable from v2 ckpt)")
print(SEP)
