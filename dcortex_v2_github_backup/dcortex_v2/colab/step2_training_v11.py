...
TC = TrainConfig()
assert abs(TC.simple_ratio + TC.update_ratio + TC.distractor_ratio - 1.0) < 1e-6

print(SEP)
print("[INFO] D_CORTEX v2.0-alpha STEP 2 v11: COMPLEX EPISODES (Objective B1)")
print(f"  Steps: {TC.total_steps} | accum: {TC.grad_accum}")
print(f"  Facts per episode: {TC.min_facts}-{TC.max_facts}")
print(f"  Structural mix: {TC.simple_ratio:.0%} simple | {TC.update_ratio:.0%} updates | "
      f"{TC.distractor_ratio:.0%} distractors")
print(f"  LM ratio: {TC.lm_ratio:.0%}")
print(f"  Lexical alpha: {TC.lexical_alpha}")
print(f"  ema_alpha=0.9 (update blending in config)")
print(SEP)
...
# Try loading v10 checkpoint as warm start
try:
    v10_ckpts = sorted(Path(CHECKPOINT_DIR).glob('ckpt_step*.pt'),
                       key=lambda p: int(p.stem.split('step')[1]))
    if v10_ckpts:
        latest = v10_ckpts[-1]
        c = torch.load(latest, map_location=DEVICE, weights_only=False)
        model.load_state_dict(c['model'], strict=False)
        print(f"[INFO] Warm-started from {latest.name} (step {c['step']})")
except Exception as e:
    print(f"[INFO] No warm start: {e}")
...
ADDR_LR_MULT = 3.0  # Reduced - warm start already has aligned shared
ENC_LR_MULT = 2.0
...
# Semantic clusters for distractor episodes
_CLUSTER_ANIMALS = ["cat","dog","bird","fish","rabbit","horse","bear","fox",
                     "lion","tiger","monkey","penguin","owl","wolf","deer"]
_CLUSTER_FANTASY = ["dragon","knight","wizard","princess","fairy","goblin","witch",
                     "pirate","giant","ghost","robot","queen","king","dwarf","elf"]
...
class EpisodeGT:
    ...
    target_fact_idx: int
    answer_token_id: int
    ep_type: str   # 'simple', 'update', 'distractor'


def generate_simple_episode(n_facts=0):
    if n_facts == 0:
...
def generate_update_episode(n_facts=0):
    """N simple facts, then ONE of those entities gets updated.
    Question asks for updated entity's LATEST color.
    """
    if n_facts == 0:
        n_facts = random.randint(TC.min_facts, TC.max_facts)
    entities = random.sample(_ENTITIES, n_facts)
    ...
    return EpisodeGT(facts, prompt, n_facts, upd_ans_tok, 'update')  # answer = latest


def generate_distractor_episode(n_facts=0):
    """Facts only from one semantic cluster (animals or fantasy).
    Tests if model distinguishes similar entities.
    """
    if n_facts == 0:
        n_facts = random.randint(TC.min_facts, TC.max_facts)
...
def generate_episode():
    ...
    if r < TC.simple_ratio:
        return generate_simple_episode()
    elif r < TC.simple_ratio + TC.update_ratio:
        return generate_update_episode()
    else:
        return generate_distractor_episode()
...
def run_structural_episode(model, ep: EpisodeGT):
    ...
    # L_sep_neg (for update: original+update share entity, skip them)
    ...
    if len(fact_keys) >= 2:
        ...
        # For update episodes, also mask the original+update pair (same entity)
...
@torch.no_grad()
def eval_type(model, ep_gen_fn, n=200):
    model.eval()
    correct = 0
    for _ in range(n):
        ep = ep_gen_fn()
        ...
        if aux_logits[0].argmax().item() == ep.answer_token_id: correct += 1
    model.train()
    return correct / n
...
def save_ckpt(model, optimizer, step, best):
    ...
    fname = f'ckpt_v11_step{step:06d}.pt'
...
def load_latest_v11(model, optimizer):
    ckpts = sorted(Path(CHECKPOINT_DIR).glob('ckpt_v11_step*.pt'),
                   key=lambda p: int(p.stem.split('step')[1]))
...
print(SEP); print("[INFO] OBJECTIVE B1: COMPLEX EPISODES"); print(SEP)

start_step, best_val = load_latest_v11(model, optimizer)
print("[INFO] Starting fresh." if start_step == 0 else f"[INFO] Resumed from step {start_step}")
...
for step in range(start_step, TC.total_steps):
    ...
    acc = {'emit': 0, 'sel': 0, 'sep_neg': 0, 'top1': 0, 'lm': 0,
           'n_simple': 0, 'n_update': 0, 'n_distractor': 0, 'n_lm': 0,
           'top1_simple': 0, 'top1_update': 0, 'top1_distractor': 0}
    ...
    if step % TC.log_every == 0 or step == start_step:
        ...
        # Per-type accuracy within batch (if any of type)
        def pct(n, k):
            return f"{100 * acc[f'top1_{k}'] / n:.0f}%" if acc[f'n_{k}'] > 0 else "--"
        print(f"Step {step:5d}/{TC.total_steps} | "
              f"emit={acc['emit']:.3f} sel={acc['sel']:.3f} lm={acc['lm']:.3f} | "
              f"top1 S={pct(acc['n_simple'], 'simple')} "
              f"U={pct(acc['n_update'], 'update')} "
              f"D={pct(acc['n_distractor'], 'distractor')} | "
              f"n={acc['n_simple']}/{acc['n_update']}/{acc['n_distractor']}/{acc['n_lm']} | "
              f"gn={grad_norm:.2f} ETA {int(eta_s//60)}m", flush=True)

    if (step + 1) % TC.eval_every == 0:
        val_loss = eval_lm(model)
        acc_s = eval_type(model, generate_simple_episode, n=200)
        acc_u = eval_type(model, generate_update_episode, n=200)
        acc_d = eval_type(model, generate_distractor_episode, n=200)
        print(f"  [EVAL] step={step+1} | val={val_loss:.3f} | "
              f"simple={acc_s:.1%} update={acc_u:.1%} distr={acc_d:.1%}", flush=True)
    ...
    if math.isnan(acc['emit']) or math.isinf(acc['emit']):
        print(f"[ERROR] NaN at step {step}"); break
...
print("\n[INFO] Final evaluation (500 episodes each):")
for name, fn in [('simple', generate_simple_episode),
                  ('update', generate_update_episode),
                  ('distractor', generate_distractor_episode)]:
    a = eval_type(model, fn, n=500)
    print(f"  {name:12s}: {a:.1%}")

# Scaling for each type
print("\n[INFO] Scaling test (n_facts) for each episode type:")
for nf in [3, 5, 7, 10]:
    nf = min(nf, len(_ENTITIES))
    # override min/max to fix n_facts
    TC.min_facts = nf; TC.max_facts = nf
    s = eval_type(model, generate_simple_episode, n=200)
    u = eval_type(model, generate_update_episode, n=200)
    d = eval_type(model, generate_distractor_episode, n=200)
    print(f"  n_facts={nf:2d}: simple={s:.1%} update={u:.1%} distr={d:.1%}")
TC.min_facts = 3; TC.max_facts = 5

print(SEP)
print("OBJECTIVE B1 CRITERIA:")
final_s = eval_type(model, generate_simple_episode, n=500)
final_u = eval_type(model, generate_update_episode, n=500)
final_d = eval_type(model, generate_distractor_episode, n=500)
print(f"  Simple: {final_s:.1%}       [{'PASS' if final_s > 0.85 else 'FAIL'}] >85%")
print(f"  Update: {final_u:.1%}       [{'PASS' if final_u > 0.75 else 'FAIL'}] >75%")
print(f"  Distr:  {final_d:.1%}       [{'PASS' if final_d > 0.75 else 'FAIL'}] >75%")
print(SEP)

report = {
    'step': 'Step 2 v11: Objective B1 - Complex Episodes',
    'best_val': best_val,
    'final_simple': final_s,
    'final_update': final_u,
    'final_distractor': final_d,
}
with open(os.path.join(RESULTS_DIR, 'step2v11_report.json'), 'w') as f:
