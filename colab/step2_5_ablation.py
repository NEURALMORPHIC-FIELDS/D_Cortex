...
@torch.no_grad()
def encode_facts(model, fact_texts):
    """Encode a list of facts, return slot_writes."""
    slot_writes = []
    for fact in fact_texts:
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            aux = model.encode(xf)
        slot_writes.append(aux['slot_writes'][0])
    return slot_writes


@torch.no_grad()
def decode_question(model, prompt, answer):
    """Decode and return logits at answer position + predicted token."""
    p_ids = ENC.encode_ordinary(prompt)
    a_ids = ENC.encode_ordinary(answer)
    xp = torch.tensor([_pad(p_ids)], dtype=torch.long, device=DEVICE)
    with torch.amp.autocast('cuda', dtype=torch.bfloat16):
        logits = model.decode(xp)
    pred_pos = min(len(p_ids) - 1, SEQ_LEN - 1)
    answer_logits = logits[0, pred_pos, :].float()
    answer_probs = F.softmax(answer_logits, dim=-1)
    target_tok = a_ids[0] if a_ids else EOT
    target_prob = answer_probs[target_tok].item()
    pred_tok = answer_logits.argmax().item()
    # Rank of target
    sorted_ids = answer_logits.argsort(descending=True)
    rank = (sorted_ids == target_tok).nonzero(as_tuple=True)[0].item() + 1
    return {
        'pred_tok': pred_tok,
...
print(f"\n{'='*70}")
print(f"ABLATION: Where does the retrieval → answer signal die?")
print(f"{'='*70}\n")
...
# Pre-generate paired episodes (A, B) so we can swap memories
episode_pairs = []
for _ in range(N_EPISODES):
    ep_A = gen_episode(n_facts=4)
    ep_B = gen_episode(n_facts=4)
    episode_pairs.append((ep_A, ep_B))

print(f"Running {N_EPISODES} episodes in 3 conditions...")

for i, (ep_A, ep_B) in enumerate(episode_pairs):
    facts_A, prompt_A, answer_A, target_A, _, _ = ep_A
    facts_B, prompt_B, answer_B, target_B, _, _ = ep_B

    # ---- CONDITION 1: NORMAL ----
    # Encode A's facts, decode A's question
    with contextlib.redirect_stdout(io.StringIO()): model.reset_memory()
    encode_facts(model, facts_A)
    results_normal.append(decode_question(model, prompt_A, answer_A))

    # ---- CONDITION 2: ZERO MEMORY ----
    # No encoding - memory stays empty
    with contextlib.redirect_stdout(io.StringIO()): model.reset_memory()
    results_zero.append(decode_question(model, prompt_A, answer_A))

    # ---- CONDITION 3: PERMUTED ----
    # Encode B's facts (unrelated to question A)
    with contextlib.redirect_stdout(io.StringIO()): model.reset_memory()
    encode_facts(model, facts_B)
    results_permuted.append(decode_question(model, prompt_A, answer_A))
...
def summarize(results, name):
    n = len(results)
    top1 = sum(1 for r in results if r['pred_tok'] == r['target_tok']) / n
    mean_rank = sum(r['rank'] for r in results) / n
    mean_prob = sum(r['target_prob'] for r in results) / n
    median_rank = sorted([r['rank'] for r in results])[n // 2]
    return {
        'name': name, 'n': n,
        'top1': top1, 'mean_rank': mean_rank,
        'median_rank': median_rank, 'mean_prob': mean_prob,
    }


def compare_logits(res_a, res_b, name_a, name_b):
    """L2 distance between logits averaged across episodes."""
    dists = []
    kls = []
    for a, b in zip(res_a, res_b):
        la, lb = a['logits'], b['logits']
        dists.append((la - lb).norm().item())
        # KL divergence (a || b)
        pa = F.softmax(la, dim=-1)
        lpa = F.log_softmax(la, dim=-1)
        lpb = F.log_softmax(lb, dim=-1)
        kl = (pa * (lpa - lpb)).sum().item()
        kls.append(kl)
    return {
        'logit_l2_mean': sum(dists) / len(dists),
        'kl_mean': sum(kls) / len(kls),
    }


norm_sum = summarize(results_normal, "Normal (correct memory)")
zero_sum = summarize(results_zero, "Zero memory (empty)")
perm_sum = summarize(results_permuted, "Permuted memory (wrong episode)")

norm_vs_zero = compare_logits(results_normal, results_zero, "normal", "zero")
norm_vs_perm = compare_logits(results_normal, results_permuted, "normal", "permuted")
zero_vs_perm = compare_logits(results_zero, results_permuted, "zero", "permuted")

print(f"\n{'='*70}")
print(f"RESULTS ({N_EPISODES} episodes, 4 facts each)")
print(f"{'='*70}\n")
print(f"  {'Condition':<40s} {'Top1':>8s} {'MeanRank':>10s} {'MedRank':>9s} {'MeanProb':>10s}")
print(f"  {'-' * 77}")
for s in [norm_sum, zero_sum, perm_sum]:
    print(f"  {s['name']:<40s} {s['top1']:>7.1%} {s['mean_rank']:>10.1f} "
          f"{s['median_rank']:>9d} {s['mean_prob']:>10.4f}")

print(f"\n{'='*70}")
print(f"LOGIT DIVERGENCE")
print(f"{'='*70}\n")
print(f"  Normal vs Zero       | L2: {norm_vs_zero['logit_l2_mean']:.3f}  "
      f"KL: {norm_vs_zero['kl_mean']:.4f}")
print(f"  Normal vs Permuted   | L2: {norm_vs_perm['logit_l2_mean']:.3f}  "
      f"KL: {norm_vs_perm['kl_mean']:.4f}")
print(f"  Zero vs Permuted     | L2: {zero_vs_perm['logit_l2_mean']:.3f}  "
      f"KL: {zero_vs_perm['kl_mean']:.4f}")
...
# Compute key ratios
top1_lift_vs_zero = norm_sum['top1'] - zero_sum['top1']
top1_lift_vs_permuted = norm_sum['top1'] - perm_sum['top1']
prob_lift_vs_zero = norm_sum['mean_prob'] - zero_sum['mean_prob']
prob_lift_vs_permuted = norm_sum['mean_prob'] - perm_sum['mean_prob']

print(f"  Accuracy lift (normal - zero):     {top1_lift_vs_zero:+.1%}")
print(f"  Accuracy lift (normal - permuted): {top1_lift_vs_permuted:+.1%}")
print(f"  Prob lift (normal - zero):         {prob_lift_vs_zero:+.4f}")
print(f"  Prob lift (normal - permuted):     {prob_lift_vs_permuted:+.4f}")
print()

if norm_vs_zero['kl_mean'] < 0.1 and norm_vs_perm['kl_mean'] < 0.1:
    verdict = "DECODER IGNORES MEMORY"
    explanation = ("Logits barely change when memory is zeroed or permuted. "
                   "Decoder is not using retrieved_value. Fix: aux_answer_head from retrieved_value.")
elif top1_lift_vs_permuted > 0.15 and norm_vs_perm['kl_mean'] > 0.5:
    verdict = "MEMORY CONTAINS SIGNAL but LANGUAGE INTERFACE WEAK"
    explanation = ("Retrieved value does contain fact-specific information. "
                   "But the path retrieved_value → answer_token is insufficient. "
                   "Fix: aux_answer_head + L_cycle to force value to be linguistically decodable.")
elif top1_lift_vs_zero > 0.3:
    verdict = "MEMORY WORKS, LM DEGRADED"
    explanation = ("Memory helps substantially. val_loss catastrophe is pure LM drift. "
                   "Fix: curriculum_ratio = 0.5-0.6 to restore language.")
else:
    verdict = "MIXED SIGNAL"
    explanation = "Results are ambiguous. Inspect per-episode distributions."
...
report = {
    ...
    'normal': norm_sum,
    'zero_memory': zero_sum,
    'permuted_memory': perm_sum,
    'divergence': {
        'normal_vs_zero': norm_vs_zero,
        'normal_vs_permuted': norm_vs_perm,
        'zero_vs_permuted': zero_vs_perm,
    },
    'verdict': verdict,
    'explanation': explanation,
}

out_path = os.path.join(PROJECT_ROOT, 'results', 'ablation_report.json')
