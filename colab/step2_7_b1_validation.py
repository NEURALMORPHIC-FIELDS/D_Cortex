...
# Cluj-Napoca, Romania
#
# D_Cortex v2.0-alpha -- Step 2.7: B1 Rigorous Validation
# Runs 6 suites beyond basic eval: scaling, update chains, semantic distractors,
# cross-cluster, held-out entities, held-out + distractors.
# Patent EP25216372.0.
...
ckpts = sorted(Path(os.path.join(PROJECT_ROOT, 'checkpoints')).glob('ckpt_v11_step*.pt'),
               key=lambda p: int(p.stem.split('step')[1]))
if not ckpts:
    raise RuntimeError("No v11 checkpoint found.")
...
_CLUSTER_ANIMALS = _ENTITIES[:15]
_CLUSTER_FANTASY = _ENTITIES[15:]

# Cross-schema: 4 different semantic classes
CROSS_SCHEMA = {
    'animal': ["cat","dog","bird","fish","rabbit","horse","bear","fox"],
    'fantasy': ["dragon","knight","wizard","princess","fairy","goblin","witch"],
    'object': ["book","sword","crown","mirror","ring","lamp","chair","door"],
    'person': ["teacher","doctor","baker","farmer","sailor","painter","runner","dancer"],
}

@torch.no_grad()
def encode_episode(facts_with_answers):
    with contextlib.redirect_stdout(io.StringIO()): model.reset_memory()
    for text, ans_tok in facts_with_answers:
        f_ids = _pad(ENC.encode_ordinary(text) + [EOT])
        xf = torch.tensor([f_ids], dtype=torch.long, device=DEVICE)
        ans_id = torch.tensor([ans_tok], dtype=torch.long, device=DEVICE)
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            model.encode(xf, answer_token_id=ans_id,
                        lexical_alpha=0.9, force_bank='working')

@torch.no_grad()
def query_and_predict(prompt):
    ...
    return aux_logits[0].argmax().item()
...
results = {'block1_scaling': {}, 'block2_updates': {}, 'block3_generalization': {}}

# ======================== BLOCK 1: SCALING ==================================

print(f"\n{'#'*72}\n# BLOCK 1: Scaling to capacity (working_mem = 16 slots)\n{'#'*72}")
print(f"  Random baseline: 1/15 (colors) = 6.7%")
print()

random.seed(42)

def run_simple(n_facts, N=500):
    correct = 0
    for _ in range(N):
        nf = min(n_facts, len(_ENTITIES))
        ents = random.sample(_ENTITIES, nf)
        cols = random.sample(_COLORS, nf)
        target = random.randint(0, nf - 1)
        facts = [(f"The {e} is {c}.", ENC.encode_ordinary(f" {c}")[0])
                  for e, c in zip(ents, cols)]
        encode_episode(facts)
        prompt = f"What color is the {ents[target]}? The {ents[target]} is"
        pred = query_and_predict(prompt)
        target_tok = ENC.encode_ordinary(f" {cols[target]}")[0]
        if pred == target_tok: correct += 1
    return correct, N

def run_update_total(total_facts, N=500):
    """total_facts = total fact count (init + N-1 distractors + 1 update).
    Target has 2 facts, N-2 distinct distractors.
    """
    correct = 0
    for _ in range(N):
        n_entities = max(2, total_facts - 1)  # N-1 distinct entities, target appears twice
        n_entities = min(n_entities, len(_ENTITIES))
        ents = random.sample(_ENTITIES, n_entities)
        target_idx = random.randint(0, n_entities - 1)
        cols = random.sample(_COLORS, n_entities + 1)
        
        facts = []
        for i, (e, c) in enumerate(zip(ents, cols[:n_entities])):
            facts.append((f"The {e} is {c}.", ENC.encode_ordinary(f" {c}")[0]))
        new_col = cols[n_entities]
        facts.append((f"The {ents[target_idx]} is now {new_col}.",
                     ENC.encode_ordinary(f" {new_col}")[0]))
        
        encode_episode(facts)
        prompt = f"What color is the {ents[target_idx]} now? The {ents[target_idx]} is"
        pred = query_and_predict(prompt)
        target_tok = ENC.encode_ordinary(f" {new_col}")[0]
        if pred == target_tok: correct += 1
    return correct, N

def run_distractor(n_facts, N=500, cluster=_CLUSTER_ANIMALS):
    correct = 0
    for _ in range(N):
        nf = min(n_facts, len(cluster))
        ents = random.sample(cluster, nf)
        cols = random.sample(_COLORS, nf)
        target = random.randint(0, nf - 1)
        facts = [(f"The {e} is {c}.", ENC.encode_ordinary(f" {c}")[0])
                  for e, c in zip(ents, cols)]
...
print(f"  {'N_facts':<10s}{'Simple':<25s}{'Update':<25s}{'Distractor':<25s}")
print(f"  {'-'*85}")
# Max 15 because we have only 15 colors; update needs n+1 colors so cap at 14 for update
for nf in [3, 5, 8, 12, 15]:
    cs, ns = run_simple(nf, N=500)
    # Update needs n+1 colors (one for the update), so cap at 14
    nf_update = min(nf, 14)
    cu, nu = run_update_total(nf_update, N=500)
    cd, nd = run_distractor(min(nf, 15), N=500)
    ps = cs / ns; pu = cu / nu; pd = cd / nd
    lo_s, hi_s = wilson_ci(cs, ns)
    lo_u, hi_u = wilson_ci(cu, nu)
    lo_d, hi_d = wilson_ci(cd, nd)
    s = f"{ps:.1%} [{lo_s:.1%}-{hi_s:.1%}]"
    u = f"{pu:.1%} [{lo_u:.1%}-{hi_u:.1%}]"
    d = f"{pd:.1%} [{lo_d:.1%}-{hi_d:.1%}]"
    print(f"  {nf:<10d}{s:<25s}{u:<25s}{d:<25s}")
    results['block1_scaling'][f'n_{nf}'] = {
        'simple': {'acc': ps, 'ci': [lo_s, hi_s], 'n': ns},
        'update': {'acc': pu, 'ci': [lo_u, hi_u], 'n': nu, 'nf_effective': nf_update},
        'distractor': {'acc': pd, 'ci': [lo_d, hi_d], 'n': nd},
    }

# ======================== BLOCK 2: UPDATE CHAINS =============================

print(f"\n{'#'*72}\n# BLOCK 2: Update chain stability\n{'#'*72}")
print(f"  Tests if ema_alpha blending degrades over successive updates.")
print(f"  Random baseline: 1/15 = 6.7%")
print()

random.seed(43)

def run_chain(chain_len, n_other=3, N=300):
    correct = 0
    for _ in range(N):
        all_ents = random.sample(_ENTITIES, n_other + 1)
        target_ent = all_ents[0]
        other_ents = all_ents[1:]
        all_colors = random.sample(_COLORS, chain_len + 1 + n_other)
        target_colors = all_colors[:chain_len + 1]
        other_colors = all_colors[chain_len + 1:]
        
        facts = []
        facts.append((f"The {target_ent} is {target_colors[0]}.",
                     ENC.encode_ordinary(f" {target_colors[0]}")[0]))
        for e, c in zip(other_ents, other_colors):
            facts.append((f"The {e} is {c}.", ENC.encode_ordinary(f" {c}")[0]))
        for i in range(chain_len):
            new_col = target_colors[i + 1]
            facts.append((f"The {target_ent} is now {new_col}.",
                         ENC.encode_ordinary(f" {new_col}")[0]))
        encode_episode(facts)
        prompt = f"What color is the {target_ent} now? The {target_ent} is"
        pred = query_and_predict(prompt)
        final_col = target_colors[-1]
        target_tok = ENC.encode_ordinary(f" {final_col}")[0]
        if pred == target_tok: correct += 1
    return correct, N

print(f"  {'Chain':<10s}{'Accuracy':<30s}{'Degradation'}")
print(f"  {'-'*60}")
baseline_acc = None
for chain_len in [1, 2, 4, 8]:
    c, n = run_chain(chain_len, N=300)
    p = c / n
    lo, hi = wilson_ci(c, n)
    if baseline_acc is None: baseline_acc = p
    degrad = (p - baseline_acc) * 100
    acc_str = f"{p:.1%} [{lo:.1%}-{hi:.1%}]"
    print(f"  {chain_len:<10d}{acc_str:<30s}{degrad:+.1f}pp")
    results['block2_updates'][f'chain_{chain_len}'] = {
        'acc': p, 'ci': [lo, hi], 'n': n, 'degradation_pp': degrad,
    }

# Leak test - does model return intermediate instead of final?
print(f"\n  Leak test at chain=3 (N=300):")
random.seed(44)
initial_leak = 0; mid_leak = 0
N = 300
for _ in range(N):
    all_ents = random.sample(_ENTITIES, 4)
    target_ent = all_ents[0]
    other_ents = all_ents[1:]
    cols = random.sample(_COLORS, 7)
    target_colors = cols[:4]
    other_colors = cols[4:]
    facts = [(f"The {target_ent} is {target_colors[0]}.",
              ENC.encode_ordinary(f" {target_colors[0]}")[0])]
    for e, c in zip(other_ents, other_colors):
        facts.append((f"The {e} is {c}.", ENC.encode_ordinary(f" {c}")[0]))
    for i in range(3):
        facts.append((f"The {target_ent} is now {target_colors[i+1]}.",
                     ENC.encode_ordinary(f" {target_colors[i+1]}")[0]))
    encode_episode(facts)
    prompt = f"What color is the {target_ent} now? The {target_ent} is"
    pred = query_and_predict(prompt)
    initial_tok = ENC.encode_ordinary(f" {target_colors[0]}")[0]
    mid1_tok = ENC.encode_ordinary(f" {target_colors[1]}")[0]
    mid2_tok = ENC.encode_ordinary(f" {target_colors[2]}")[0]
    if pred == initial_tok: initial_leak += 1
    elif pred in [mid1_tok, mid2_tok]: mid_leak += 1
print(f"    Returned initial value:      {initial_leak}/{N} = {initial_leak/N:.1%}")
print(f"    Returned intermediate value: {mid_leak}/{N} = {mid_leak/N:.1%}")
results['block2_updates']['leak_test'] = {'initial': initial_leak/N, 'intermediate': mid_leak/N, 'n': N}

# ======================== BLOCK 3: GENERALIZATION ============================

print(f"\n{'#'*72}\n# BLOCK 3: Generalization (rare entities + cross-schema)\n{'#'*72}")
print(f"  NOTE: True held-out requires retraining. Here we test:")
print(f"    3.1 Rare entities (last 5 of 30 - seen but less frequent)")
print(f"    3.2 Cross-schema (mix of 4 semantic classes in one episode)")
print(f"    3.3 Cross-schema with distractors from different classes")
print(f"  For true held-out exclusion, a v12 retraining is needed.")
print()

# --- 3.1 Rare entities ---
print(f"  3.1 Rare entities (last 5 of 30 entity pool):")
random.seed(45)
correct = 0; N = 300
RARE = _ENTITIES[-5:]
for _ in range(N):
    ents = random.sample(RARE, len(RARE))  # Use all 5 rare
    cols = random.sample(_COLORS, len(ents))
    target = random.randint(0, len(ents) - 1)
    facts = [(f"The {e} is {c}.", ENC.encode_ordinary(f" {c}")[0])
              for e, c in zip(ents, cols)]
    encode_episode(facts)
    prompt = f"What color is the {ents[target]}? The {ents[target]} is"
...
p = correct/N
print(f"    Rare-only (5 entities): {p:.1%} [{lo:.1%}-{hi:.1%}]  (N={N})")
results['block3_generalization']['rare_only'] = {'acc': p, 'ci': [lo, hi], 'n': N, 'entities': RARE}

# --- 3.1b Rare target among frequent distractors ---
print(f"  3.1b Rare target among frequent distractors:")
random.seed(46)
correct = 0; N = 300
FREQUENT = _ENTITIES[:25]
for _ in range(N):
    target_ent = random.choice(RARE)
    distractors = random.sample(FREQUENT, 4)
    ents = [target_ent] + distractors
    random.shuffle(ents)
    cols = random.sample(_COLORS, len(ents))
    target_idx = ents.index(target_ent)
    facts = [(f"The {e} is {c}.", ENC.encode_ordinary(f" {c}")[0])
              for e, c in zip(ents, cols)]
    encode_episode(facts)
    prompt = f"What color is the {target_ent}? The {target_ent} is"
    pred = query_and_predict(prompt)
    target_tok = ENC.encode_ordinary(f" {cols[target_idx]}")[0]
    if pred == target_tok: correct += 1
lo, hi = wilson_ci(correct, N)
p = correct/N
print(f"    Rare target + 4 freq distractors: {p:.1%} [{lo:.1%}-{hi:.1%}]  (N={N})")
results['block3_generalization']['rare_with_freq_distractors'] = {'acc': p, 'ci': [lo, hi], 'n': N}

# --- 3.2 Cross-schema mix ---
print(f"\n  3.2 Cross-schema (1 from each of 4 classes):")
random.seed(47)
correct = 0; N = 500
for _ in range(N):
    classes = list(CROSS_SCHEMA.keys())
    random.shuffle(classes)
    ents = [random.choice(CROSS_SCHEMA[c]) for c in classes]
    cols = random.sample(_COLORS, 4)
...
lo, hi = wilson_ci(correct, N)
p = correct/N
print(f"    Cross-schema (animal+fantasy+object+person): {p:.1%} [{lo:.1%}-{hi:.1%}]  (N={N})")
print(f"    NOTE: object/person classes were NOT in v11 training data.")
print(f"    Low accuracy here = evidence of compositional limit (expected).")
results['block3_generalization']['cross_schema_4class'] = {'acc': p, 'ci': [lo, hi], 'n': N}

# --- 3.3 Cross-schema trained classes only (animal + fantasy mix) ---
print(f"\n  3.3 Cross-schema (animal+fantasy only, both in training):")
random.seed(48)
correct = 0; N = 500
for _ in range(N):
    n_ani = random.randint(2, 3)
    n_fan = random.randint(2, 3)
    ents = (random.sample(_CLUSTER_ANIMALS, n_ani) + 
             random.sample(_CLUSTER_FANTASY, n_fan))
    random.shuffle(ents)
    cols = random.sample(_COLORS, len(ents))
    target = random.randint(0, len(ents) - 1)
    facts = [(f"The {e} is {c}.", ENC.encode_ordinary(f" {c}")[0])
...
lo, hi = wilson_ci(correct, N)
p = correct/N
print(f"    Cross-trained-cluster: {p:.1%} [{lo:.1%}-{hi:.1%}]  (N={N})")
results['block3_generalization']['cross_trained_clusters'] = {'acc': p, 'ci': [lo, hi], 'n': N}

# ======================== FINAL REPORT ======================================

print(f"\n{'='*72}")
print(f"B1.1 VALIDATION REPORT")
print(f"{'='*72}")
print(f"  Checkpoint: {ckpts[-1].name} (step {ckpt_step})")
print(f"\n  Block 1 - Scaling (simple at 3/15 facts): "
      f"{results['block1_scaling']['n_3']['simple']['acc']:.1%} -> "
      f"{results['block1_scaling']['n_15']['simple']['acc']:.1%}")
print(f"  Block 1 - Scaling (update at 3/15 facts): "
      f"{results['block1_scaling']['n_3']['update']['acc']:.1%} -> "
      f"{results['block1_scaling']['n_15']['update']['acc']:.1%}")
print(f"  Block 2 - Chain 1/4/8: "
      f"{results['block2_updates']['chain_1']['acc']:.1%} / "
      f"{results['block2_updates']['chain_4']['acc']:.1%} / "
      f"{results['block2_updates']['chain_8']['acc']:.1%}")
print(f"  Block 3 - Rare only: {results['block3_generalization']['rare_only']['acc']:.1%}")
print(f"  Block 3 - Cross-schema 4class: {results['block3_generalization']['cross_schema_4class']['acc']:.1%}")
print(f"  Block 3 - Cross-trained-cluster: {results['block3_generalization']['cross_trained_clusters']['acc']:.1%}")
print(f"{'='*72}")

with open(os.path.join(PROJECT_ROOT, 'results', 'b1_1_validation.json'), 'w') as f:
