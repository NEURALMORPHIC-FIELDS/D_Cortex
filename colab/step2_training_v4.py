# -*- coding: utf-8 -*-
# ===========================================================================
# D_Cortex v2.0-alpha -- Step 2: Training + Memory Validation (v4)
...
#
# CHANGES vs v3:
#   [FIX-9] MEMORY-DOMINANT CURRICULUM: 85% memory-essential batches from
#           step 0, 15% generic LM as regularization. No ramp, no warmup.
#           Memory tasks: delayed recall, contradiction, update chain,
#           entity confusion traps, archive retrieval.
#
#   [FIX-10] KEY DIVERSITY LOSS: penalizes mean pairwise cosine similarity
#            between occupied slots in state, episode_obj, working banks.
#            Forces new slot allocation instead of EMA on one slot.
#            L_div = mean(cos(k_i, k_j)) over occupied pairs, weight=0.05.
#
#   [FIX-11] THETA_MATCH raised from 0.7 to 0.85 in inline source.
#            Higher threshold = harder to match existing slots = more
#            new slot allocations = higher bank occupancy.
#
# ROOT CAUSE (from v3 A100 run):
#   key-query alignment converged perfectly (ka=0.000) but all queries
#   from mean-pooled TinyStories produce near-identical keys. With
#   theta_match=0.7, every write matched the same slot -> 1 slot per bank.
#   Bank occupancy: 1/64 state, 1/128 episode_obj, 1/32 conflict.
#   Suite A delta: +12.5% (first non-zero, but 1/8 facts).
#
# All v2 fixes (FIX 1-7) and v3 fix (FIX-8) remain in effect.
# ===========================================================================
...
fig.suptitle(
    'D_Cortex v2.0-alpha -- Step 2 (v4): Training + Memory Aux Losses',
    fontsize=14, y=0.98,
)
...
plt.tight_layout()
plot_path = os.path.join(RESULTS_DIR, 'step2v4_training_report.png')
...
report = {
    ...
    'step': 'Step 2 v4: Training with writer gradient fix (key-query alignment + value coherence)',
...
report_path = os.path.join(RESULTS_DIR, 'step2v4_training_report.json')
with open(report_path, 'w', encoding='utf-8') as f:
    json.dump(report, f, indent=2, default=str)

print(SEP)
print("D_CORTEX v2.0-alpha -- STEP 2 v4 REPORT")
print(SEP)
print(f"  Fixes applied   : 11 (FIX-1..8 from v2/v3 + FIX-9/10/11 in v4)")
...
print(f"  Curriculum      : {TC.curriculum_ratio:.0%} memory-essential from step 0")
print(f"  theta_match     : 0.85 (was 0.7 in v2/v3)")
...
if M_div:
    print(f"  Key diversity   : {M_div[0]:.4f} -> {M_div[-1]:.4f} (w={TC.w_diversity})")
...
# Bank occupancy (the critical metric for v4)
...
print(SEP)
print("v4 CHANGES (vs v3 which had 1-slot-per-bank):")
print(f"  FIX-9  : curriculum {TC.curriculum_ratio:.0%} memory-essential from step 0")
print(f"  FIX-10 : key diversity loss (w={TC.w_diversity}) on state/episode_obj/working")
print(f"  FIX-11 : theta_match raised to 0.85 (harder to match existing slots)")
print()
print("WHAT TO LOOK FOR:")
print("  1. Bank occupancy >> 1 per bank (was 1/64, 1/128, 1/32 in v3)")
print("  2. Key diversity loss should be LOW (keys are spread out)")
print("  3. Gate distribution should be NON-UNIFORM")
print("  4. Fusion mem_gate should move away from 0.5")
print("  5. Same-batch ablation delta > 0")
print("  6. Step 3 Suite A delta > 12.5% (was +12.5% in v3 with 1 slot)")
print("  7. Step 3 Suite B conflict_slots > 0")
print(SEP)
